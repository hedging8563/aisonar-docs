...
<ResponseField name="data" type="array">
  ...
  - `tokenlab.category` (string): Kategori model publik
  - `tokenlab.pricing_unit` (string atau `null`): Satuan harga publik jika tersedia
  - `tokenlab.capability_flags` (object atau `null`): Ringkasan kapabilitas publik
  - `tokenlab.supported_operations` (array atau `null`): Operasi non-chat publik untuk model
  - `tokenlab.pricing_provenance` (object atau `null`): Metadata sumber harga publik
  - `tokenlab.public_contract_summary` (object atau `null`): Ringkasan discovery non-chat ringan yang hanya berisi `public_operations` dan `request_endpoint`
  - `tokenlab.agent_preferences` (object): Hanya dikembalikan saat `recommended_for` ada
  - `tokenlab.lifecycle` (object): tahap lifecycle, tanggal rilis/depresiasi, model pengganti, dan sumber badge latest
  - `tokenlab.commercial` (object): kebijakan biaya pengguna, alasan gratis, dan waktu gratis hingga opsional
  - `tokenlab.badges` (array): badge tampilan dari metadata lifecycle dan commercial
</ResponseField>
