...
<ParamField body="model" type="string" default="veo3.1">
  ID model video. Gunakan ID logis tingkat produk seperti `veo3.1`, `wan-2.7`, `happyhorse-1.0`, `viduq3`, `pixverse-v6`, atau `kling-3.0-video`; pilih text-to-video, image-to-video, reference-to-video, atau varian lain dengan `operation`. Lihat [panduan video](/id/guides/video-generation) dan [Models API](/id/api-reference/models/list-models).
</ParamField>
...
Baca `tokenlab.capabilities`, `tokenlab.supported_operations`, `tokenlab.public_contract_summary`, dan `tokenlab.public_contract` dari respons detail model. Operasi seperti `audio-to-video` dan `video-extension` bersifat khusus model; konfirmasi ketersediaan terbarunya di sana, bukan dari contoh statis di halaman ini.
