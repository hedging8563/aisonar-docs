...
Baca `aisonar.capabilities`, `aisonar.supported_operations`, `aisonar.public_contract_summary`, dan `aisonar.public_contract` dari respons detail model. Operasi seperti `audio-to-video` dan `video-extension` bersifat khusus model; konfirmasi ketersediaan terbarunya di sana, bukan dari contoh statis di halaman ini.
