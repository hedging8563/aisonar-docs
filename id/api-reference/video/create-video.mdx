...
<ParamField body="model" type="string" default="veo3.1">
  ID model video. Gunakan ID model yang ditampilkan AI Sonar seperti `veo3.1`, `wan-2.7`, `happyhorse-1.0`, `viduq3`, `pixverse-v6`, atau `kling-3.0-video`; pilih text-to-video, image-to-video, reference-to-video, atau varian lain dengan `operation`. Lihat [panduan video](/id/guides/video-generation) dan [Models API](/id/api-reference/models/list-models).
</ParamField>
...
<ParamField body="reference_images" type="array">
  Gambar referensi untuk alur dengan conditioning khusus. Jumlah yang didukung bergantung pada model. Untuk `seedance-2.0` dan `seedance-2.0-fast`, AI Sonar saat ini mendukung hingga 9 gambar referensi, ditambah hingga 3 video referensi dan 3 audio referensi. Untuk pilihan model, batas 4K, dan catatan Mini, lihat [panduan model video Seedance 2.0](/id/guides/seedance-2-video). URL publik `https` lebih disarankan; model yang kompatibel juga menerima URL `data:`. Untuk `grok-imagine-video`, reference-to-video menerima hingga 7 referensi gambar dan `duration` dibatasi maksimal 10 detik. `grok-imagine-video-1.5-preview` hanya mendukung image-to-video dan tidak menerima referensi gambar.
</ParamField>

<ParamField body="material_asset_id" type="string">
  ID aset AI Sonar Seedance yang dikembalikan oleh [Buat aset](/id/api-reference/video/create-material-asset). Gunakan dengan model Seedance 2.0 setelah aset berstatus `ACTIVE`.
</ParamField>

<ParamField body="material_asset_ids" type="array">
  Beberapa ID aset AI Sonar Seedance. Batasnya sama dengan `reference_images`; untuk Seedance 2.0, kirim total maksimal 9 referensi gambar.
</ParamField>
...
<ParamField body="output_audio" type="boolean">
  Toggle output audio bergantung pada model. Di AI Sonar, request keluarga Veo 3 default ke `true` saat field ini dihilangkan. `kling-3.0-video` menerima selector ini untuk request tanpa referensi elemen dan memetakannya ke kontrol suara yang kompatibel untuk model tersebut; request Kling yang menghilangkan field ini defaultnya silent. Jangan gabungkan `output_audio=true` dengan `kling_elements`. Model video lain menggunakan default yang terdokumentasi. Alias camelCase `outputAudio` diterima untuk kompatibilitas.
</ParamField>
...
Gunakan `operation=reference-to-video` ketika model mendukung conditioning referensi khusus. Dalam detail model AI Sonar, referensi gambar menggunakan `reference_images`, sedangkan video dan audio referensi multimodal menggunakan `video_urls` dan `audio_urls`. Untuk `seedance-2.0` dan `seedance-2.0-fast`, AI Sonar saat ini mendukung hingga 9 gambar referensi, ditambah hingga 3 video referensi dan 3 audio referensi. Untuk pilihan model, batas 4K, dan catatan Mini, lihat [panduan model video Seedance 2.0](/id/guides/seedance-2-video). `duration` hanya mengatur panjang output yang dihasilkan; field ini tidak menetapkan batas terpisah untuk durasi input video referensi. Untuk `grok-imagine-video`, reference-to-video menerima hingga 7 referensi gambar (`reference_images` atau `image_urls`) dan `duration` dibatasi maksimal 10 detik. Jangan gabungkan referensi gambar dengan input frame awal `image_url` / `image`. `grok-imagine-video-1.5-preview` hanya mendukung image-to-video.
