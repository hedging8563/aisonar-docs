...
description: "Pilih endpoint gambar AI Sonar yang tepat, kirim permintaan yang memahami model, dan tangani hasil gambar sinkron atau asinkron dengan aman."
---

AI Sonar mendukung teks-ke-gambar, gambar-ke-gambar, dan pengeditan gambar melalui endpoint gambar publik. Model gambar tidak berbagi satu set parameter universal, jadi klien produksi harus terlebih dahulu memilih endpoint, kemudian memilih model, lalu mengirim hanya bidang yang didukung oleh model tersebut.
...
Mulailah dengan penemuan model, kemudian periksa kontrak AI Sonar model yang dipilih:
...
Untuk model non-obrolan, respon daftar mungkin mencakup `aisonar.public_contract_summary`. Halaman detail model mungkin mengekspos `aisonar.public_contract` yang lebih lengkap. Gunakan bidang tersebut untuk mengonfirmasi:
...
- Permintaan gaya `gpt-image-2` menggunakan bidang `size`, `quality`, dan edit yang mirip dengan OpenAI. Tinggalkan bidang opsional saat Anda ingin model atau AI Sonar menggunakan default otomatis.
...
- Validasi input pengguna sebelum memanggil AI Sonar: panjang prompt, jumlah gambar, keterjangkauan URL, dan jenis file.
