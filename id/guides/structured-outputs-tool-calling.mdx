...
Keluaran terstruktur dan panggilan alat adalah kontrak spesifik rute. AI Sonar meneruskan beberapa format API publik, jadi pola produksi yang paling aman adalah menjaga setiap bentuk alat asli penyedia pada rute yang memilikinya dan memvalidasi keluaran model di aplikasi Anda sendiri.
...
Jangan kirim alat bawaan asli penyedia melalui rute yang berbeda dan berharap AI Sonar akan menerjemahkannya secara diam-diam.
...
Validasi bersama yang stabil untuk Chat Completions menerima `text` dan `json_object`. `json_schema`, `strict`, dan penegakan skema spesifik penyedia mungkin ada untuk beberapa rute hulu atau jalur konversi, tetapi mereka bukan janji umum di setiap rute dan model AI Sonar. Verifikasi mereka terhadap model yang dipilih sebelum mengandalkannya.
...
AI Sonar tidak mengeksekusi fungsi Anda. Aplikasi Anda memiliki loop:
