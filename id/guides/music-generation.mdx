...
Contoh publik saat ini menggunakan `suno_music` untuk generasi musik dan menyertakan `mv` dengan versi model Suno resmi seperti `chirp-v4`. Untuk alur hanya lirik, kirim `action: "LYRICS"` dengan model yang dokumen detail modelnya mencantumkan generasi lirik dan hilangkan `mv`. Perlakukan ID model sebagai ID AI Sonar publik, bukan sebagai jaminan bahwa bidang spesifik penyedia adalah bidang detail model.
...
Simpan prompt, judul, dan tag yang terlihat oleh pengguna dan aman untuk disimpan. Jangan tempatkan kunci API, URL pribadi, atau informasi diagnostik pribadi di bidang prompt mana pun.
...
| Parameter tidak didukung | Bidang tidak ada dalam detail model model | Hapus bidang spesifik penyedia atau pilih model yang mendokumentasikannya |
