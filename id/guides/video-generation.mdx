...
Kontrak video publik AI Sonar menerima nilai operasi berikut. Dukungan bersifat spesifik per model dan berubah saat penyedia menambah atau menghentikan kemampuan, jadi baca kontrak model yang dipilih sebelum bergantung pada operasi khusus.
...
- **I2V (Image-to-Video)**: menganimasikan gambar awal. Untuk kompatibilitas terluas, gunakan `image_url`.
- **Reference**: mengondisikan generasi dengan satu atau beberapa gambar referensi melalui `reference_images`; beberapa model juga menerima video referensi melalui `video_urls` dan audio referensi melalui `audio_urls`.
- **Start-End**: mengontrol frame pertama dan terakhir dengan `start_image` dan `end_image`.
- **V2V (Video-to-Video)**: menggunakan video yang sudah ada, task yang telah dibuat, atau alur turunan khusus penyedia sebagai sumber.
- **Motion**: menggabungkan gambar subjek dengan video referensi gerakan.
- **Audio-to-Video**: menghasilkan video dari alur model yang dikondisikan oleh audio.
- **Video Extension**: melanjutkan atau memperpanjang task pembuatan video yang sudah ada.

## Penemuan model

Ketersediaan model video sering berubah. Ambil shortlist publik terbaru sebelum memilih model:
...
Baca model yang dipilih sebelum mengirim field khusus model:
...
Gunakan `aisonar.capabilities`, `aisonar.supported_operations`, `aisonar.public_contract_summary`, dan `aisonar.public_contract` sebagai sumber kebenaran. Contoh di bawah adalah pola workflow, bukan inventaris model lengkap.
...
| `video_url` | string | Wajib untuk alur `video-to-video` berbasis URL video dan untuk `motion-control`; beberapa alur turunan menggunakan `task_id` sebagai gantinya. |
...
<CardGroup cols={2}>
  <Card title="Kualitas tertinggi" icon="crown">
    Jika kualitas lebih penting daripada kecepatan, **veo3.1**, **kling-video-o1-pro**, dan **viduq3-pro** adalah pilihan yang kuat.
  </Card>
  ...
  <Card title="Video ke video" icon="film">
    Mulai dari `GET /v1/models?recommended_for=video`; contoh bergaya V2V saat ini mencakup **grok-imagine-video**, **seedance-2.0**, **veo3.1**, dan **kling-video-o1-pro / std**.
  </Card>
</CardGroup>
