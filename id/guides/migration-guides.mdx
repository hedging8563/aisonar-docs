...
description: "Pindahkan OpenAI, Anthropic, Gemini, dan beban kerja media ke AI Sonar dengan perubahan kecil yang aman untuk produksi."
---

AI Sonar adalah multi-format: Anda dapat mempertahankan klien yang kompatibel dengan OpenAI, panggilan Messages asli Anthropic, panggilan REST asli Gemini, dan titik akhir media dalam bentuk aslinya. Migrasi yang paling aman adalah tidak menerjemahkan setiap beban kerja ke dalam satu format universal. Pilih rute yang memiliki perilaku yang dibutuhkan aplikasi Anda.

## Pemetaan Rute

| Beban kerja yang ada | URL dasar AI Sonar | Titik akhir utama | Catatan migrasi |
|----------------------|--------------------|-------------------|-----------------|
| OpenAI Chat Completions | `https://api.aisonar.dev/v1` | `/chat/completions` | Perubahan terkecil untuk chat dan pemanggilan fungsi yang kompatibel dengan OpenAI |
| OpenAI Responses | `https://api.aisonar.dev/v1` | `/responses` | Gunakan ketika aplikasi Anda bergantung pada input, alat, atau penanganan output yang spesifik untuk Responses |
| Anthropic SDK | `https://api.aisonar.dev` | `/v1/messages` | Jangan tambahkan `/v1` ke URL dasar SDK |
| Gemini REST | `https://api.aisonar.dev` | `/v1beta/models/:model:generateContent` | Pertahankan bidang asli Gemini di rute Gemini |
| Generasi media | `https://api.aisonar.dev/v1` | `/images`, `/videos`, `/music`, `/3d` | Temukan model dengan `recommended_for` dan harapkan polling asinkron di mana didokumentasikan |
| Manajemen dan penagihan | `https://api.aisonar.dev/v1` | `/management/...` | Gunakan token manajemen untuk penggunaan sisi server dan rekonsiliasi penagihan |
...
2. Baca `aisonar.public_contract_summary` dalam respons daftar dan `aisonar.public_contract` lengkap di mana tersedia.
...
- Jangan bandingkan penagihan dengan ID tugas penyedia. Gunakan catatan penggunaan AI Sonar.
