...
公開影片模型庫存與支援操作會持續變化。接入某個模型特定流程前，請以 [Models API](/zh-Hant/api-reference/models/list-models) 作為真值來源：
...
請讀取單模型詳情回應中的 `tokenlab.capabilities`、`tokenlab.supported_operations`、`tokenlab.public_contract_summary` 和 `tokenlab.public_contract`。`audio-to-video`、`video-extension` 等操作屬於模型特定能力；請在那裡確認即時可用性，不要依賴本頁的靜態範例。
