...
請讀取單模型詳情回應中的 `aisonar.capabilities`、`aisonar.supported_operations`、`aisonar.public_contract_summary` 和 `aisonar.public_contract`。`audio-to-video`、`video-extension` 等操作屬於模型特定能力；請在那裡確認即時可用性，不要依賴本頁的靜態範例。
