---
title: 'API 參考'
description: 'AI Sonar API 的完整參考'
---
...
AI Sonar API 是**原生優先，同時相容 OpenAI** 的。需要供應商原生行為時，使用 Anthropic 的 `POST /v1/messages` 或 Gemini 的 `/v1beta/models/...:generateContent`；遷移已有 OpenAI 風格 SDK 或工具時，使用 OpenAI 相容的 `/v1` 端點。`POST /v1/responses` 仍是需要 Responses 特定行為時的進階可選路徑。
...
在 [控制台](https://console.aisonar.dev) 取得您的 API key。
...
<Info>
使用 `/v1/realtime` 發起 WebSocket 升級請求。一般 `GET /v1/realtime` 會回傳端點資訊，方便無法直接檢查 WebSocket 路由的用戶端使用。
</Info>
