...
<ParamField body="contents" type="array" required>
  ...
  對於媒體 part，AI Sonar 目前接受圖片、音訊、影片三類 MIME type，並透過 Gemini 相容支援說明向下游轉發。

  `user` 和 `model` role 會以不區分大小寫的方式正規化。`application/octet-stream` 的 `inlineData` / `inline_data` 只有在 AI Sonar 能辨識為支援的圖片或影片位元組時才會接受，否則會在路由前失敗。原生圖片輸出請求只接受 Google search/maps 工具族，不支援的工具組合會在處理開始前失敗。
</ParamField>
