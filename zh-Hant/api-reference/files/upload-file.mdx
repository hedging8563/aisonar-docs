...
此端點是 AI Sonar 的 OpenAI 相容批次 API 的一部分。批次作業使用上傳的 JSONL 檔案，異步運行，並且可以稍後返回輸出/錯誤檔案。
...
<ResponseField name="purpose" type="string">API 返回的事件或訊息類型。</ResponseField>
<ResponseField name="bytes" type="integer">用於計費對帳的 USD 十進位字串。</ResponseField>
