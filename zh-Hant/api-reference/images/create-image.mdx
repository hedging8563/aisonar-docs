...
<ParamField body="size" type="string" default="1024x1024">
  ...
  對於 `gpt-image-2`，`size` 接受 `auto` 或 `WIDTHxHEIGHT`。自訂寬高都必須是 16 的倍數，最長邊不超過 `3840px`，長邊/短邊比例不超過 `3:1`，總像素必須介於 `655,360` 到 `8,294,400` 之間。`aspect_ratio` 與 `resolution` 目前不屬於 AI Sonar 對 `gpt-image-2` 的対応状況。
