...
<ParamField body="response_format" type="string" default="url">
  ...
  對於 Azure Official 或 Azure-compatible 的 `gpt-image-2` 請求，AI Sonar 會以 `b64_json` 接收圖片資料。請求 `url` 時，AI Sonar 會把每張圖上傳到 CDN 並返回 `data[].url`。如果 CDN 儲存不可用或上傳失敗，請求會失敗，而不是改為 Base64 回應；請求 `b64_json` 時直接返回原始 Base64。
</ParamField>
