...
`gpt-image-2` 是按 token 計費的 GPT Image 模型。AI Sonar 依 OpenAI 官方 usage 明細結算文字輸入、圖片輸入、已回報的快取輸入與圖片輸出 token；它不是固定每張圖片計費的模型。

對於 `gpt-image-2` 圖像生成，公共參數支援 `prompt`、`n`、`size`、`quality`、`response_format`、`async`、`background`、`output_format`、`output_compression` 或 `compression`、`moderation`、`partial_images` 和 `user`。未傳 `size` 或 `quality` 時 AI Sonar 會使用 `auto`；自訂 `size` 必須使用下方說明的彈性 `WIDTHxHEIGHT` 契約。
...
- `nano-banana-2` 在目前 AI Sonar 契約中僅文字生圖支援 `aspect_ratio` 和 `resolution`（`1k`、`2k`、`4k`）。
...
- Nano Banana 圖生圖請求中，`nano-banana-pro` 可以傳送 `resolution`（`1k`、`2k`、`4k`）；`nano-banana-edit` 必須省略此參數。`nano-banana` 和 `nano-banana-2` 在目前公開契約中是文字生圖模型。
...
xAI Grok Imagine 圖片模型（`grok-imagine-image`、`grok-imagine-image-quality` 以及 legacy `grok-imagine-image-pro`）支援 `aspect_ratio` 與 `resolution`（`1k`、`2k`）。`grok-imagine-image-pro` 會保留為相容 ID，並在 upstream 路由至 `grok-imagine-image-quality`。
...
<ParamField body="size" type="string" default="1024x1024">
  ...
  對於 `gpt-image-2`，`size` 接受 `auto` 或 `WIDTHxHEIGHT`。自訂寬高都必須是 16 的倍數，最長邊不超過 `3840px`，長邊/短邊比例不超過 `3:1`，總像素必須介於 `655,360` 到 `8,294,400` 之間。`aspect_ratio` 與 `resolution` 目前不屬於 AI Sonar 對 `gpt-image-2` 的公開契約。
...
<ResponseField name="data" type="array">
  ...
  - `revised_prompt` (string): 上游模型回傳的選填 prompt 改寫結果（僅在提供者回傳時出現）
</ResponseField>
