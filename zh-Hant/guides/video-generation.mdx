...
AI Sonar 公開影片契約接受以下操作值。支援情況與具體模型有關，並會隨供應商新增或下線能力而變化；依賴某個專用操作前，請先讀取所選模型契約。
...
- **Reference**：透過 `reference_images` 傳入一張或多張參考圖進行條件控制；部分模型也透過 `video_urls` 接受參考影片、透過 `audio_urls` 接受參考音訊。
- **Start-End**：透過 `start_image` 與 `end_image` 控制首幀與尾幀。
- **V2V（Video-to-Video）**：以現有影片、已生成任務或供應商特定衍生流程作為來源。
- **Motion**：同時使用主體圖片與動作參考影片。
- **Audio-to-Video**：透過音訊條件控制的模型流程生成影片。
- **Video Extension**：繼續或延展既有影片生成任務。

## 模型探索

影片模型可用性變化很快。選擇模型前，先取得目前公開影片推薦清單：
...
請以 `aisonar.capabilities`、`aisonar.supported_operations`、`aisonar.public_contract_summary` 與 `aisonar.public_contract` 作為真值來源。以下範例是工作流模式，不是完整模型庫存。
...
| `video_url` | string | 基於影片 URL 的 `video-to-video` 流程與 `motion-control` 需要此欄位；部分衍生流程改用 `task_id`。 |
...
<CardGroup cols={2}>
  <Card title="最高品質" icon="crown">
    若品質優先於速度，可優先考慮 **veo3.1**、**kling-video-o1-pro** 或 **viduq3-pro**。
  </Card>
  ...
  <Card title="影片轉影片" icon="film">
    先呼叫 `GET /v1/models?recommended_for=video`；目前 V2V 類範例包括 **grok-imagine-video**、**seedance-2.0**、**veo3.1** 與 **kling-video-o1-pro / std**。
  </Card>
</CardGroup>
