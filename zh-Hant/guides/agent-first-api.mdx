...
AI Sonar 的 Agent-First API 在錯誤回應中加入了可結構化的提示，AI 代理可以立即解析並採取行動——不需網路搜尋、不需查文件、也不需猜測。

每個錯誤回應的標準 `error` 物件內都可能包含像 `did_you_mean`、`suggestions`、`hint`、`retryable` 和 `retry_after` 類型的可選欄位。這些欄位向後相容——不使用它們的客戶端不會看到差異。
...
<Note>
與 OpenAI 相容的端點使用 AI Sonar 穩定的公開錯誤類型，例如 `rate_limit_exceeded`、`upstream_error` 和 `all_channels_failed`。與 Anthropic 相容與 Gemini 相容的端點則使用它們各自的原生回應格式。
</Note>
...
| `cache_pricing` | object or `null` | 僅在模型存在上游 prompt cache 價格時回傳。 |
