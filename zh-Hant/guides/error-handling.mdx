...
基礎欄位 (`message`, `type`, `code`, `param`) 永遠會存在。提示欄位 (`did_you_mean`, `suggestions`, `hint`, `retryable`, `retry_after`, `balance_usd`, `estimated_cost_usd`) 為 AI agent 自我修正的選用擴充。詳情請參閱 [Agent-First API guide](/guides/agent-first-api)。

OpenAI 相容 endpoints 使用 AI Sonar 的穩定 gateway 錯誤類型。Anthropic-compatible 與 Gemini-compatible endpoints 則使用其原生的錯誤分類與回應格式。
...
公開路由在回應主體中不會區分拼字錯誤、隱藏、延後啟用或非公開的模型狀態。如果模型目前無法透過公開合約取得，AI Sonar 會回傳 `model_not_found`。
