---
title: "可觀察性與故障排除"
description: "記錄支援所需的公共 AI Sonar 識別碼、使用情況對帳和非同步媒體除錯。"
---

良好的 AI Sonar 可觀察性始於公共識別碼。您的日誌應該幫助您回答「用戶請求了什麼，AI Sonar 創建了什麼公共任務，以及如何計費？」而不暴露敏感供應商細節或敏感的用戶數據。
...
| 您自己的工作/用戶 ID | 您的應用程式 | 將 AI Sonar 活動與用戶工作流程關聯 |
...
`GET /v1/management/api-keys/{keyId}/usage` 可以根據 `scene`、`accessChannel`、`model`、`modelVendor`、`startDate` 和 `endDate` 進行過濾。使用這些記錄，而不是抓取儀表板頁面或依賴供應商任務 ID。
...
除非 AI Sonar 支援明確要求提供已隱去的範本，否則請勿包括 API 鍵、管理令牌、私有媒體、完整提示、供應商 URL、通道 ID 或私有調試識別碼。
