...
| **Gemini CLI** | 盡力支援 / 實驗性 | Gemini | 自訂的 AI Sonar base URL 流程不是官方 Gemini CLI 整合 |
| **OpenCode** | Supported | OpenAI 相容 | 預設使用 OpenAI 相容的 provider；僅在您明確需要時，才改用以 Responses 為基礎的 provider |

<Note>
  其他 OpenAI 相容編輯器與 Agent 工具通常也可使用相同的 base URL 模式；正式使用前請先確認該工具自身支援自訂 provider。
</Note>
