---
title: "遷移指南"
description: "將 OpenAI、Anthropic、Gemini 和媒體工作負載以小型、適合生產的變更移動到 AI Sonar。"
---

AI Sonar 是多格式的：您可以保持 OpenAI 兼容的客戶端、Anthropic 原生的 Messages 調用、Gemini 原生的 REST 調用，以及媒體端點的自然形狀。最安全的遷移方式不是將每個工作負載轉換為一種通用格式。選擇擁有您的應用程序所需行為的路徑。

## 路徑映射

| 現有工作負載 | AI Sonar 基本 URL | 主要端點 | 遷移說明 |
|-------------------|-------------------|------------------|----------------|
| OpenAI 聊天完成 | `https://api.aisonar.dev/v1` | `/chat/completions` | 對於 OpenAI 兼容的聊天和函數調用，變更最小 |
| OpenAI 回應 | `https://api.aisonar.dev/v1` | `/responses` | 當您的應用依賴於特定於回應的輸入、工具或輸出處理時使用 |
| Anthropic SDK | `https://api.aisonar.dev` | `/v1/messages` | 不要在 SDK 基本 URL 後附加 `/v1` |
| Gemini REST | `https://api.aisonar.dev` | `/v1beta/models/:model:generateContent` | 在 Gemini 路徑上保持 Gemini 原生字段 |
| 媒體生成 | `https://api.aisonar.dev/v1` | `/images`, `/videos`, `/music`, `/3d` | 使用 `recommended_for` 發現模型，並在文檔中預期異步輪詢 |
| 管理和計費 | `https://api.aisonar.dev/v1` | `/management/...` | 對於伺服器端使用和計費對賬，使用管理令牌 |
...
2. 在列表響應中閱讀 `aisonar.public_contract_summary` 和可用的完整 `aisonar.public_contract`。
3. 對於圖像端點，明確發送 `model`。
...
- 不要使用供應商任務 ID 來比較計費。使用 AI Sonar 使用記錄。
