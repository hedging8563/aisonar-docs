...
當前的公共示例使用 `suno_music` 進行音樂生成，並在 `mv` 中傳入 `chirp-v4` 等官方 Suno 模型版本。對於僅歌詞的流程，發送 `action: "LYRICS"` 與模型詳情文件中包含歌詞生成的模型，並省略 `mv`。將模型 ID 視為公共 AI Sonar ID，而不是保證供應商特定字段是支持说明字段。
...
保持提示、標題和標籤對用戶可見並安全存儲。不要在任何提示字段中放置 API 密鑰、私有 URL 或私有調試信息。
...
| 不支持的參數 | 字段不在模型支持说明中 | 刪除供應商特定字段或選擇一個記錄它們的模型 |
