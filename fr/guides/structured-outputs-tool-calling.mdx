...
Ne pas envoyer d'outils intégrés natifs du fournisseur par une route différente et s'attendre à ce que AI Sonar les traduise silencieusement.
...
La validation partagée stable pour les Chat Completions accepte `text` et `json_object`. `json_schema`, `strict`, et l'application de schémas spécifiques au fournisseur peuvent exister pour certaines routes en amont ou chemins de conversion, mais ils ne constituent pas une promesse générale sur chaque route et modèle de AI Sonar. Vérifiez-les par rapport au modèle sélectionné avant de vous y fier.
...
AI Sonar n'exécute pas vos fonctions. Votre application possède la boucle :
