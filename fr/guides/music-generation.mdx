...
Les exemples publics actuels utilisent `suno_music` pour la génération de musique et incluent `mv` avec une version officielle du modèle Suno, comme `chirp-v4`. Pour les flux uniquement de paroles, envoyez `action: "LYRICS"` avec un modèle dont le détails du modèle documente la génération de paroles et omettez `mv`. Traitez les ID de modèle comme des ID publics AI Sonar, et non comme une garantie que les champs spécifiques au fournisseur sont des champs de détails du modèle.
...
Gardez les prompts, titres et tags visibles par l'utilisateur et sûrs à stocker. Ne placez pas de clés API, d'URLs privées ou d'informations de diagnostic privées dans les champs de prompt.
...
| Paramètre non pris en charge | Le champ n'est pas dans le détails du modèle du modèle | Supprimez les champs spécifiques au fournisseur ou choisissez un modèle qui les documente |
