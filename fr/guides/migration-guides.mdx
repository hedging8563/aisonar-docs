...
description: "Déplacez OpenAI, Anthropic, Gemini et les charges de travail multimédia vers AI Sonar avec de petits changements sûrs pour la production."
---

AI Sonar est multi-format : vous pouvez conserver des clients compatibles avec OpenAI, des appels Messages natifs à Anthropic, des appels REST natifs à Gemini et des points de terminaison multimédia dans leurs formes naturelles. La migration la plus sûre consiste à ne pas traduire chaque charge de travail en un format universel. Choisissez la route qui possède le comportement dont votre application a besoin.

## Cartographie des Routes

| Charge de travail existante | URL de base AI Sonar | Point de terminaison principal | Remarque sur la migration |
|-----------------------------|----------------------|-------------------------------|--------------------------|
| OpenAI Chat Completions     | `https://api.aisonar.dev/v1` | `/chat/completions`          | Changement le plus petit pour le chat compatible avec OpenAI et l'appel de fonction |
| OpenAI Responses            | `https://api.aisonar.dev/v1` | `/responses`                 | Utilisez lorsque votre application dépend d'entrées, d'outils ou de gestion de sortie spécifiques aux Responses |
| Anthropic SDK               | `https://api.aisonar.dev`    | `/v1/messages`               | Ne pas ajouter `/v1` à l'URL de base du SDK |
| Gemini REST                 | `https://api.aisonar.dev`    | `/v1beta/models/:model:generateContent` | Conservez les champs natifs de Gemini sur la route Gemini |
...
| Gestion et facturation       | `https://api.aisonar.dev/v1` | `/management/...`            | Utilisez des tokens de gestion pour un usage côté serveur et la réconciliation de facturation |
...
2. Lisez `aisonar.public_contract_summary` dans les réponses de liste et le `aisonar.public_contract` complet lorsque disponible.
...
- Ne comparez pas la facturation avec les ID de tâche du fournisseur. Utilisez les enregistrements d'utilisation de AI Sonar.
