...
description: "Choisissez le bon point de terminaison d'image AI Sonar, envoyez des requêtes conscientes du modèle et gérez les résultats d'image synchrones ou asynchrones en toute sécurité."
---

AI Sonar prend en charge la génération d'images à partir de texte, d'images à partir d'images et l'édition d'images via des points de terminaison d'image publics. Les modèles d'image ne partagent pas un ensemble de paramètres universel, donc les clients de production doivent d'abord choisir le point de terminaison, puis choisir le modèle, puis envoyer uniquement les champs pris en charge par ce modèle.
...
Commencez par la découverte de modèles, puis inspectez le contrat AI Sonar du modèle sélectionné :
...
Pour les modèles non liés aux chats, la réponse de la liste peut inclure `aisonar.public_contract_summary`. Les pages de détails des modèles peuvent exposer le `aisonar.public_contract` complet. Utilisez ces champs pour confirmer :
...
- Les requêtes de style `gpt-image-2` utilisent des champs `size`, `quality`, et d'édition similaires à OpenAI. Laissez de côté les champs optionnels lorsque vous souhaitez que le modèle ou AI Sonar utilise des valeurs par défaut automatiques.
...
- Validez les entrées utilisateur avant d'appeler AI Sonar : longueur du prompt, nombre d'images, accessibilité des URLs, et type de fichier.
