---
title: "Codex CLI"
description: "Configurer OpenAI Codex CLI pour utiliser l'API AI Sonar"
---
...
Pour AI Sonar, Codex CLI peut utiliser `/v1/responses`, mais vous devez le considérer comme un chemin de compatibilité avancé. Certaines fonctionnalités spécifiques à Responses ne sont pas garanties sur tous les modèles et chemins routés.
...
<Warning>
Codex déprécie la prise en charge de `chat/completions` pour les fournisseurs personnalisés. Conservez `wire_api = "responses"` pour AI Sonar à moins que vous n'utilisiez intentionnellement un ancien chemin de compatibilité.
</Warning>

<Note>
Si une requête utilise des champs spécifiques à Responses qui ne sont pas pris en charge par le modèle ou le chemin sélectionné, AI Sonar retourne une erreur explicite au lieu de rétrograder silencieusement la requête.
</Note>
...
<AccordionGroup>
  <Accordion title="Erreur de connexion">
    - Vérifiez que `base_url` dans config.toml est exactement `https://api.aisonar.dev/v1`
  ...
  <Accordion title="Modèle introuvable">
    ...
    - Vérifiez la disponibilité du modèle sur [aisonar.dev/en/models](https://aisonar.dev/models)
  </Accordion>

  <Accordion title="Champ natif Responses rejeté">
    - Certains champs ne sont disponibles que sur `/v1/responses` lorsque AI Sonar peut garantir ce comportement pour le modèle et le chemin sélectionnés
  ...
  <Accordion title="/compact ou l'auto-compactage échoue">
    ...
    - Conservez `wire_api = "responses"` et `base_url = "https://api.aisonar.dev/v1"`
  </Accordion>
</AccordionGroup>
