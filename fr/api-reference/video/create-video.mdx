...
<ParamField body="model" type="string" default="veo3.1">
  ID du modele video. Utilisez les IDs de modèle affichés par AI Sonar comme `veo3.1`, `wan-2.7`, `happyhorse-1.0`, `viduq3`, `pixverse-v6` ou `kling-3.0-video`; choisissez text-to-video, image-to-video, reference-to-video ou d autres variantes avec `operation`. Voir le [guide video](/fr/guides/video-generation) et [Models API](/fr/api-reference/models/list-models).
</ParamField>
...
<ParamField body="operation" type="string">
  Opération vidéo à exécuter. Les valeurs acceptées sont `text-to-video`, `image-to-video`, `reference-to-video`, `start-end-to-video`, `video-to-video`, `video-extension`, `audio-to-video` et `motion-control`. AI Sonar peut déduire l’opération à partir des entrées, mais une valeur explicite est recommandée en production.
</ParamField>
...
<ParamField body="material_asset_id" type="string">
  ID de ressource AI Sonar Seedance retourné par [Créer une ressource](/fr/api-reference/video/create-material-asset). Utilisez-le avec les modèles Seedance 2.0 une fois la ressource `ACTIVE`.
</ParamField>

<ParamField body="material_asset_ids" type="array">
  Plusieurs ID de ressources AI Sonar Seedance. Ils partagent la même limite de références image que `reference_images`; pour Seedance 2.0, envoyez au plus 9 références image au total.
</ParamField>
...
<ParamField body="output_audio" type="boolean">
  Bascule de sortie audio dépendante du modèle. Dans AI Sonar, les requêtes de la famille Veo 3 utilisent `true` par défaut lorsque ce champ est omis. `kling-3.0-video` accepte ce sélecteur pour les requêtes sans références d’éléments et le mappe vers le contrôle sonore compatible de ce modèle ; les requêtes Kling omises sont silencieuses par défaut. Ne combinez pas `output_audio=true` avec `kling_elements`. Les autres modèles vidéo utilisent leurs valeurs par défaut documentées. L’alias camelCase `outputAudio` est accepté pour compatibilité.
</ParamField>
...
Quand un modèle exige à la fois une image de sujet et une vidéo de mouvement de référence, utilisez `operation=motion-control`. AI Sonar normalise la forme publique `image_url` + `video_url` vers le format motion-control de ce modèle.
