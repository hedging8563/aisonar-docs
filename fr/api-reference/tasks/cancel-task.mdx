...
L’annulation est prise en charge pour les tâches vidéo Seedance en file d’attente lorsque le fournisseur sélectionné pour cette tâche prend en charge l’annulation. Les modèles concernés sont `seedance-1.5-pro`, `seedance-2.0` et `seedance-2.0-fast`. Les tâches déjà en `processing`/running ne peuvent pas être annulées. Les tâches terminées, échouées, expirées ou non prises en charge restent consultables pour le statut et le rapprochement de facturation.
