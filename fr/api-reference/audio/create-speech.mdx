...
<ParamField body="model" type="string" default="tts-1">
  Modele TTS. Les exemples compatibles OpenAI incluent `tts-1` (standard) et `tts-1-hd` (qualite superieure); la voix haute definition MiniMax est disponible avec `speech-2.8-hd`. Consultez `GET /v1/models?recommended_for=tts` pour la selection actuelle.
</ParamField>
