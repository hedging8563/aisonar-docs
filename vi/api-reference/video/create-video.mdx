...
Đọc `aisonar.capabilities`, `aisonar.supported_operations`, `aisonar.public_contract_summary` và `aisonar.public_contract` trong response chi tiết model. Các thao tác như `audio-to-video` và `video-extension` phụ thuộc từng model; hãy xác nhận trạng thái hiện tại ở đó thay vì dựa vào ví dụ tĩnh trên trang này.
