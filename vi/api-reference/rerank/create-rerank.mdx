...
<ParamField body="top_n" type="integer">
  Số lượng kết quả hàng đầu cần trả về. Mặc định là tất cả tài liệu. Phải tối thiểu là `1` và không được lớn hơn `documents.length`. Hiện AI Sonar chưa áp dụng giới hạn công khai thấp hơn theo từng provider; nếu giới hạn này thay đổi, trang này sẽ được cập nhật trước khi áp dụng.
</ParamField>
