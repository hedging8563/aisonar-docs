...
description: "Di chuyển OpenAI, Anthropic, Gemini và các khối lượng công việc truyền thông sang AI Sonar với những thay đổi nhỏ, an toàn cho sản xuất."
---

AI Sonar hỗ trợ nhiều định dạng: bạn có thể giữ lại các khách hàng tương thích với OpenAI, các cuộc gọi Messages gốc của Anthropic, các cuộc gọi REST gốc của Gemini và các điểm cuối truyền thông ở dạng tự nhiên của chúng. Việc di chuyển an toàn nhất là không dịch mọi khối lượng công việc thành một định dạng chung. Chọn lộ trình phù hợp với hành vi mà ứng dụng của bạn cần.

## Bản Đồ Lộ Trình

| Khối lượng công việc hiện tại | URL cơ sở AI Sonar | Điểm cuối chính | Ghi chú di chuyển |
|-------------------|-------------------|------------------|----------------|
| OpenAI Chat Completions | `https://api.aisonar.dev/v1` | `/chat/completions` | Thay đổi nhỏ nhất cho cuộc trò chuyện tương thích với OpenAI và gọi hàm |
| OpenAI Responses | `https://api.aisonar.dev/v1` | `/responses` | Sử dụng khi ứng dụng của bạn phụ thuộc vào đầu vào, công cụ hoặc xử lý đầu ra cụ thể cho Responses |
| Anthropic SDK | `https://api.aisonar.dev` | `/v1/messages` | Không thêm `/v1` vào URL cơ sở của SDK |
| Gemini REST | `https://api.aisonar.dev` | `/v1beta/models/:model:generateContent` | Giữ các trường gốc của Gemini trên lộ trình Gemini |
| Tạo phương tiện | `https://api.aisonar.dev/v1` | `/images`, `/videos`, `/music`, `/3d` | Khám phá các mô hình với `recommended_for` và mong đợi việc kiểm tra bất đồng bộ nơi có tài liệu |
| Quản lý và thanh toán | `https://api.aisonar.dev/v1` | `/management/...` | Sử dụng mã thông báo quản lý cho việc sử dụng phía máy chủ và đối chiếu thanh toán |
...
2. Đọc `aisonar.public_contract_summary` trong các phản hồi danh sách và toàn bộ `aisonar.public_contract` nơi có sẵn.
...
- Không so sánh thanh toán với ID công việc của nhà cung cấp. Sử dụng các bản ghi sử dụng của AI Sonar.
