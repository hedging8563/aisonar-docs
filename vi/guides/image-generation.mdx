...
Bắt đầu với việc khám phá mô hình, sau đó kiểm tra hợp đồng AI Sonar của mô hình đã chọn:
...
Đối với các mô hình không trò chuyện, phản hồi danh sách có thể bao gồm `aisonar.public_contract_summary`. Các trang chi tiết mô hình có thể tiết lộ `aisonar.public_contract` đầy đủ hơn. Sử dụng các trường đó để xác nhận:
...
- Các yêu cầu kiểu `gpt-image-2` sử dụng các trường `size`, `quality`, và chỉnh sửa giống như OpenAI. Bỏ qua các trường tùy chọn khi bạn muốn mô hình hoặc AI Sonar sử dụng các giá trị mặc định tự động.
...
- Xác thực đầu vào của người dùng trước khi gọi AI Sonar: độ dài lời nhắc, số lượng hình ảnh, khả năng tiếp cận URL, và loại tệp.
