...
Các ví dụ công khai hiện tại sử dụng `suno_music` cho việc tạo nhạc và bao gồm `mv` với phiên bản mô hình Suno chính thức như `chirp-v4`. Đối với các quy trình chỉ lời bài hát, gửi `action: "LYRICS"` với một mô hình mà chi tiết model của nó tài liệu hóa việc tạo lời bài hát và bỏ qua `mv`. Xem ID mô hình là ID AI Sonar công khai, không phải là đảm bảo rằng các trường cụ thể của nhà cung cấp là trường chi tiết model.
...
Giữ cho các prompt, tiêu đề và thẻ hiển thị với người dùng và an toàn để lưu trữ. Không đặt API keys, URL riêng tư hoặc thông tin chẩn đoán riêng tư vào bất kỳ trường prompt nào.
...
| Tham số không được hỗ trợ | Trường không có trong chi tiết model của mô hình | Xóa các trường cụ thể của nhà cung cấp hoặc chọn một mô hình tài liệu hóa chúng |
