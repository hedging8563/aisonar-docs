...
Die Basisfelder (`message`, `type`, `code`, `param`) sind immer vorhanden. Die Hinweisfelder (`did_you_mean`, `suggestions`, `hint`, `retryable`, `retry_after`, `balance_usd`, `estimated_cost_usd`) sind optionale Erweiterungen zur Selbstkorrektur von AI-Agenten. Siehe den [Agent-First API-Leitfaden](/guides/agent-first-api) für Details.

OpenAI-kompatible Endpunkte verwenden AI Sonar's stabile Gateway-Fehlertypen. Anthropic-kompatible und Gemini-kompatible Endpunkte verwenden ihre eigenen nativen Fehlerfamilien und Antwortformate.
...
Öffentliche Routen unterscheiden im Antwortkörper nicht zwischen Tippfehlern, versteckten, zurückgestellten oder nicht-öffentlichen Modellzuständen. Wenn ein Modell derzeit nicht über den öffentlichen Vertrag verfügbar ist, gibt AI Sonar `model_not_found` zurück.
