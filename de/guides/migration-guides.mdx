...
description: "Bewege OpenAI, Anthropic, Gemini und Medien-Workloads zu AI Sonar mit kleinen, produktionssicheren Änderungen."
---

AI Sonar ist mehrformatig: Sie können OpenAI-kompatible Clients, Anthropic-native Messages-Aufrufe, Gemini-native REST-Aufrufe und Medienendpunkte in ihren natürlichen Formen beibehalten. Die sicherste Migration besteht darin, nicht jede Arbeitslast in ein universelles Format zu übersetzen. Wählen Sie den Weg, der das Verhalten besitzt, das Ihre Anwendung benötigt.

## Routenabbildung

| Vorhandene Arbeitslast | AI Sonar Basis-URL | Primärer Endpunkt | Migrationshinweis |
|-----------------------|---------------------|-------------------|-------------------|
| OpenAI Chat-Vervollständigungen | `https://api.aisonar.dev/v1` | `/chat/completions` | Kleinste Änderung für OpenAI-kompatible Chats und Funktionsaufrufe |
| OpenAI Antworten | `https://api.aisonar.dev/v1` | `/responses` | Verwenden Sie, wenn Ihre App von antwortenspezifischen Eingaben, Werkzeugen oder Ausgabeverarbeitung abhängt |
| Anthropic SDK | `https://api.aisonar.dev` | `/v1/messages` | Fügen Sie `/v1` nicht zur SDK-Basis-URL hinzu |
| Gemini REST | `https://api.aisonar.dev` | `/v1beta/models/:model:generateContent` | Behalten Sie Gemini-native Felder auf der Gemini-Route |
...
| Verwaltung und Abrechnung | `https://api.aisonar.dev/v1` | `/management/...` | Verwenden Sie Verwaltungstoken für serverseitige Nutzung und Abrechnungsabgleich |
...
2. Lesen Sie `aisonar.public_contract_summary` in Listenantworten und den vollständigen `aisonar.public_contract`, wo verfügbar.
...
- Vergleichen Sie die Abrechnung nicht mit Anbieter-Task-IDs. Verwenden Sie AI Sonar-Nutzungsaufzeichnungen.
