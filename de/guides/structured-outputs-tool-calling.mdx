...
Senden Sie keine provider-native integrierten Tools über eine andere Route und erwarten Sie, dass AI Sonar sie stillschweigend übersetzt.
...
Die stabile gemeinsame Validierung für Chat Completions akzeptiert `text` und `json_object`. `json_schema`, `strict` und provider-spezifische Schema-Durchsetzung können für einige upstream-Routen oder Konvertierungspfade existieren, aber sie sind kein allgemeines Versprechen für jede AI Sonar-Route und jedes Modell. Überprüfen Sie sie gegen das ausgewählte Modell, bevor Sie sich darauf verlassen.
...
AI Sonar führt Ihre Funktionen nicht aus. Ihre Anwendung besitzt die Schleife:
