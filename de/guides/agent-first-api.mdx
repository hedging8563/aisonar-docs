...
Die Agent-First API von AI Sonar ergänzt Fehlermeldungen um strukturierte Hinweise, die KI-Agenten sofort parsen und umsetzen können — keine Websuchen, keine Dokumentationsrecherche, kein Rätselraten.
...
<Note>
OpenAI-kompatible Endpunkte verwenden AI Sonar's stabile öffentliche Fehlertypen wie `rate_limit_exceeded`, `upstream_error` und `all_channels_failed`. Anthropic-kompatible und Gemini-kompatible Endpunkte verwenden ihre eigenen nativen Antwortformen.
</Note>
...
```bash
GET https://api.aisonar.dev/v1/models?category=chat          # Chat-Modelle only
GET https://api.aisonar.dev/v1/models?category=image         # Modelle zur Bildgenerierung
GET https://api.aisonar.dev/v1/models?tag=coding&category=chat  # Auf Programmierung optimierte Chat-Modelle
```
