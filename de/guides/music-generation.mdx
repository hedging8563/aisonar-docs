...
Die aktuellen öffentlichen Beispiele verwenden `suno_music` für die Musikgenerierung und enthalten `mv` mit einer offiziellen Suno-Modellversion wie `chirp-v4`. Für reine Textflüsse senden Sie `action: "LYRICS"` mit einem Modell, dessen unterstützte Operationen die Liedgenerierung dokumentiert, und lassen `mv` weg. Behandeln Sie Modell-IDs als öffentliche AI Sonar-IDs, nicht als Garantie, dass anbieter-spezifische Felder öffentliche Vertragsfelder sind.
...
Halten Sie Prompts, Titel und Tags benutzersichtbar und sicher speicherbar. Platzieren Sie keine API-Schlüssel, privaten URLs oder privaten Diagnoseinformationen in einem Prompt-Feld.
...
| Nicht unterstützter Parameter | Feld ist nicht im Modelldetails des Modells | Entfernen Sie anbieter-spezifische Felder oder wählen Sie ein Modell, das diese dokumentiert |
