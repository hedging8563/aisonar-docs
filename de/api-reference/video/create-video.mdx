...
<ParamField body="operation" type="string">
  Auszuführende Video-Operation. Unterstützte Werte sind `text-to-video`, `image-to-video`, `reference-to-video`, `start-end-to-video`, `video-to-video`, `video-extension`, `audio-to-video` und `motion-control`. AI Sonar kann die Operation aus den Eingaben ableiten, aber in Produktion wird eine explizite Angabe empfohlen.
</ParamField>
...
<ParamField body="material_asset_id" type="string">
  AI Sonar Seedance-Material-ID aus [Material erstellen](/de/api-reference/video/create-material-asset). Verwende sie mit Seedance 2.0-Generierungsmodellen, sobald das Material `ACTIVE` ist.
</ParamField>

<ParamField body="material_asset_ids" type="array">
  Mehrere AI Sonar Seedance-Material-IDs. Sie teilen sich dasselbe Seedance-Bildreferenzlimit wie `reference_images`; für Seedance 2.0 sind insgesamt höchstens 9 Bildreferenzen erlaubt.
</ParamField>
...
<ParamField body="output_audio" type="boolean">
  Modellabhängiger Schalter für Audioausgabe. In AI Sonar verwenden Anfragen der Veo 3-Familie standardmäßig `true`, wenn dieses Feld fehlt. `kling-3.0-video` akzeptiert diesen Selector für Anfragen ohne Elementreferenzen und mappt ihn auf die kompatible Soundsteuerung dieses Modells; ausgelassene Kling-Anfragen sind standardmäßig stumm. Kombinieren Sie `output_audio=true` nicht mit `kling_elements`. Andere Videomodelle verwenden ihre dokumentierten Standardwerte. Der camelCase-Alias `outputAudio` wird aus Kompatibilitätsgründen akzeptiert.
</ParamField>
...
Wenn ein Modell sowohl ein Motivbild als auch ein Bewegungsreferenzvideo benötigt, verwenden Sie `operation=motion-control`. AI Sonar normalisiert die öffentliche Form `image_url` + `video_url` in das Motion-Control-Anfrageformat dieses Modells.
