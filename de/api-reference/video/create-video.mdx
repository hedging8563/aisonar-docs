...
<ParamField body="model" type="string" default="veo3.1">
  Video-Modell-ID. Der API-Standardwert ist `veo3.1`. Aktuelle öffentliche Videomodell-Fähigkeiten finden Sie im [Leitfaden zur Video-Generierung](/de/guides/video-generation) und in der [Models API](/de/api-reference/models/list-models).
</ParamField>
...
Lesen Sie `aisonar.capabilities`, `aisonar.supported_operations`, `aisonar.public_contract_summary` und `aisonar.public_contract` aus der Modelldetail-Antwort. Operationen wie `audio-to-video` und `video-extension` sind modellspezifisch; prüfen Sie die aktuelle Verfügbarkeit dort, statt sich auf statische Beispiele auf dieser Seite zu verlassen.
