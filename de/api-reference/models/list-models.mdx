...
<ResponseField name="data" type="array">
  ...
  - `tokenlab.pricing_unit` (string oder `null`): Öffentliche Preiseinheit, sofern verfügbar
  - `tokenlab.capability_flags` (object oder `null`): Öffentliche Fähigkeitsübersicht
  - `tokenlab.supported_operations` (array oder `null`): Öffentliche Nicht-Chat-Operationen für das Modell
  - `tokenlab.pricing_provenance` (object oder `null`): Öffentliche Metadaten zur Preisquelle
  - `tokenlab.public_contract_summary` (object oder `null`): Leichte Discovery-Zusammenfassung mit nur `public_operations` und `request_endpoint`
