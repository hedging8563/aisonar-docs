---
title: "LangChain"
description: "Integrieren Sie AI Sonar mit LangChain unter Verwendung standardmäßiger OpenAI-kompatibler Chat- und Embeddings-Funktionen"
---

## Überblick

AI Sonar funktioniert gut mit den LangChain-Integrationen `ChatOpenAI` und `OpenAIEmbeddings`, wenn Sie bei der standardmäßigen OpenAI-kompatiblen Oberfläche für Chat und Embeddings bleiben.
...
response = llm.invoke("Explain AI Sonar in one sentence.")
...
texts = [
    "AI Sonar provides one API for many AI models.",
    "AI Sonar supports OpenAI-compatible integrations."
]
...
response = rag_chain.invoke("What does AI Sonar provide?")
print(response.content)
```
...
result = executor.invoke({"input": "Search for AI Sonar pricing"})
print(result["output"])
```
...
<AccordionGroup>
  <Accordion title="`base_url` explizit übergeben">
    Die zuverlässigste AI Sonar-Konfiguration besteht darin, `base_url="https://api.aisonar.dev/v1"` direkt an `ChatOpenAI` und `OpenAIEmbeddings` zu übergeben, anstatt von älteren Aliassen für Umgebungsvariablen abhängig zu sein.
  </Accordion>
