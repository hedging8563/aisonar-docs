...
description: "Installieren Sie den gepflegten AI Sonar API Integration Skill und helfen Sie Coding Agents, Modelle zu entdecken, öffentliche Verträge zu lesen und API-Fehler zu beheben."
---

<Note>
Diese Seite dokumentiert den gepflegten gemeinsamen `aisonar-api-integration` Skill. Die kanonische Distribution liegt in [hedging8563/aisonar-skills](https://github.com/hedging8563/aisonar-skills); das öffentliche Repository enthält bewusst nur diesen Skill.
</Note>
...
- Erstellt minimal lauffähige Beispiele für AI Sonar Chat, Bilder, Audio, Video, Übersetzung und weitere API-Familien.
- Nutzt `https://api.aisonar.dev/v1` für OpenAI-kompatible Clients und erklärt den Wechsel zu nativen Anthropic- oder Gemini-Routen.
- Entdeckt Modelle über `/v1/models`, `/llms.txt` und `recommended_for`-Auswahllisten statt über veraltete Listen.
- Liest vor dem erneuten Versuch von Nicht-Chat-Anfragen den öffentlichen Modellvertrag, damit nicht unterstützte Felder nicht still entfernt werden.
...
Damit wird der gemeinsame `aisonar-api-integration` Skill aus dem AI Sonar skills Repository installiert.

Wenn Ihr Tool den Installer nicht unterstützt, kopieren Sie `skills/aisonar-api-integration/` aus dem Repository in das gemeinsame skills- oder rules-Verzeichnis des Tools.
</Note>

### Bestehende Installationen aktualisieren

Wenn Sie den Skill vor der Bereinigung des AI Sonar Repository installiert haben, führen Sie denselben Befehl erneut aus. Das aktuelle öffentliche Paket ist kleiner und hängt nicht mehr von generierten lokalen Suchskripten ab.
...
Wenn `aisonar-api-integration` erscheint, war die Installation erfolgreich.

## API Key abrufen

<Steps>
  <Step title="AI Sonar besuchen">
    Öffnen Sie [aisonar.dev](https://aisonar.dev)
  </Step>
  <Step title="Anmelden">
    Erstellen Sie ein Konto oder melden Sie sich an
  </Step>
  <Step title="API Key abrufen">
    Öffnen Sie [Dashboard → API Keys](https://console.aisonar.dev/api) und erstellen Sie einen neuen key
  </Step>
...
2. Wenn die Modellwahl offen ist, rufen Sie vor dem Hardcoding `/v1/models` oder `https://api.aisonar.dev/llms.txt` auf.
...
4. Vor Änderungen an einer fehlgeschlagenen Nicht-Chat-Anfrage lesen Sie `/v1/models/:model` und gleichen `supported_operations`, `supported_parameters`, `request_endpoint`, `request_shape_mode` und `recommended_request` ab.
...
Dieses Beispiel verwendet das OpenAI Python SDK mit der AI Sonar base URL:
...
# Vor dem erneuten Versuch einer Nicht-Chat-Anfrage den öffentlichen Vertrag eines Modells lesen
...
Der OpenAI-kompatible Pfad `/v1` ist der Standard für gängige Chat-, Responses-, Bild-, Embedding-, Audio- und Rerank-Beispiele. Nutzen Sie native Anthropic- oder Gemini-Routen nur, wenn die Anfrage anbieterspezifisches Verhalten benötigt oder AI Sonar the native endpoint hint header als Optimierungshinweis zurückgibt.
...
| Nicht-Chat-Vertragskonflikt | `supported_operations`, `supported_parameters`, `request_endpoint`, `request_shape_mode`, `recommended_request` | Anfrage aus dem öffentlichen Vertrag neu bauen und intent-verändernde Felder fail closed behandeln |
...
<AccordionGroup>
  <Accordion title="Skill wird nicht automatisch ausgelöst?">
    Erwähnen Sie “AI Sonar” oder “AI Sonar API” in der Anfrage, zum Beispiel: `Nutze AI Sonar, um Bildgenerierung in meine Node.js-App einzubauen`.
  </Accordion>
...
<Info>
Fragen? Nutzen Sie [GitHub Issues](https://github.com/hedging8563/aisonar-skills/issues) oder kontaktieren Sie [support@aisonar.dev](mailto:support@aisonar.dev).
</Info>
