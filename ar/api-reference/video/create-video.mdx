...
اقرأ `aisonar.capabilities` و `aisonar.supported_operations` و `aisonar.public_contract_summary` و `aisonar.public_contract` من استجابة تفاصيل النموذج. العمليات مثل `audio-to-video` و `video-extension` خاصة بكل نموذج؛ تحقق من توفرها الحالي هناك بدل الاعتماد على أمثلة ثابتة في هذه الصفحة.
