...
<ParamField body="model" type="string" default="veo3.1">
  معرف نموذج الفيديو. استخدم معرفات منطقية على مستوى المنتج مثل `veo3.1` أو `wan-2.7` أو `happyhorse-1.0` أو `viduq3` أو `pixverse-v6` أو `kling-3.0-video`؛ واختر text-to-video أو image-to-video أو reference-to-video أو غيرها عبر `operation`. راجع [دليل إنشاء الفيديو](/ar/guides/video-generation) و[Models API](/ar/api-reference/models/list-models).
</ParamField>
...
يتغير مخزون نماذج الفيديو العامة والعمليات المدعومة مع الوقت. استخدم [واجهة Models API](/ar/api-reference/models/list-models) كمصدر للحقيقة قبل ربط أي تدفق خاص بنموذج معين:
...
اقرأ `tokenlab.capabilities` و `tokenlab.supported_operations` و `tokenlab.public_contract_summary` و `tokenlab.public_contract` من استجابة تفاصيل النموذج. العمليات مثل `audio-to-video` و `video-extension` خاصة بكل نموذج؛ تحقق من توفرها الحالي هناك بدل الاعتماد على أمثلة ثابتة في هذه الصفحة.
