...
<ParamField body="model" type="string" default="veo3.1">
  معرف نموذج الفيديو. استخدم معرفات النماذج التي تعرضها AI Sonar مثل `veo3.1` أو `wan-2.7` أو `happyhorse-1.0` أو `viduq3` أو `pixverse-v6` أو `kling-3.0-video`؛ واختر text-to-video أو image-to-video أو reference-to-video أو غيرها عبر `operation`. راجع [دليل إنشاء الفيديو](/ar/guides/video-generation) و[Models API](/ar/api-reference/models/list-models).
</ParamField>
...
<ParamField body="operation" type="string">
  عملية الفيديو المطلوب تنفيذها. يدعم القيم `text-to-video` و `image-to-video` و `reference-to-video` و `start-end-to-video` و `video-to-video` و `video-extension` و `audio-to-video` و `motion-control`. يمكن لـ AI Sonar استنتاج العملية من المدخلات المرسلة، لكن يُنصح بإرسال `operation` بشكل صريح في بيئات الإنتاج.
</ParamField>
...
<ParamField body="reference_images" type="array">
  صور مرجعية للتدفقات التي تدعم التكييف المرجعي المخصص. العدد المدعوم يعتمد على النموذج. بالنسبة إلى `seedance-2.0` و `seedance-2.0-fast`، تدعم AI Sonar حاليًا ما يصل إلى 9 صور مرجعية، بالإضافة إلى ما يصل إلى 3 مقاطع فيديو مرجعية و3 ملفات صوت مرجعية. لاختيار النموذج وحدود 4K وملاحظات Mini، راجع [دليل نماذج فيديو Seedance 2.0](/ar/guides/seedance-2-video). يُفضَّل استخدام روابط `https` العامة، كما تقبل النماذج المتوافقة أيضًا روابط `data:`. بالنسبة إلى `grok-imagine-video`، تقبل reference-to-video حتى 7 مراجع صور ويكون `duration` بحد أقصى 10 ثوانٍ. يدعم `grok-imagine-video-1.5-preview` image-to-video فقط ولا يقبل مراجع الصور.
</ParamField>

<ParamField body="material_asset_id" type="string">
  معرّف مادة AI Sonar Seedance الذي يعيده [إنشاء مادة](/ar/api-reference/video/create-material-asset). استخدمه مع نماذج Seedance 2.0 بعد أن تصبح المادة `ACTIVE`.
</ParamField>

<ParamField body="material_asset_ids" type="array">
  عدة معرّفات لمواد AI Sonar Seedance. تشترك مع `reference_images` في حد مراجع الصور في Seedance؛ في Seedance 2.0 أرسل 9 مراجع صور كحد أقصى إجمالاً.
</ParamField>
...
<ParamField body="output_audio" type="boolean">
  مفتاح إخراج الصوت يعتمد على النموذج. في AI Sonar، طلبات عائلة Veo 3 تكون `true` افتراضيا عند حذف هذا الحقل. يقبل `kling-3.0-video` هذا المحدد للطلبات التي لا تستخدم مراجع العناصر ويحوّله إلى تحكم الصوت المتوافق لذلك النموذج؛ وعند الحذف تكون طلبات Kling صامتة افتراضيا. لا تجمع `output_audio=true` مع `kling_elements`. تستخدم نماذج الفيديو الأخرى الإعدادات الافتراضية الموثقة لها. يتم قبول الاسم camelCase `outputAudio` للتوافق.
</ParamField>
...
استخدم `operation=reference-to-video` عندما يدعم النموذج تكييفًا مرجعيًا مخصصًا. في تفاصيل النموذج لـ AI Sonar تستخدم مراجع الصور `reference_images`، بينما تستخدم الفيديوهات والصوتيات المرجعية متعددة الوسائط الحقول `video_urls` و `audio_urls`. بالنسبة إلى `seedance-2.0` و `seedance-2.0-fast`، تدعم AI Sonar حاليًا ما يصل إلى 9 صور مرجعية، بالإضافة إلى ما يصل إلى 3 مقاطع فيديو مرجعية و3 ملفات صوت مرجعية. لاختيار النموذج وحدود 4K وملاحظات Mini، راجع [دليل نماذج فيديو Seedance 2.0](/ar/guides/seedance-2-video). يتحكم `duration` فقط في طول الإخراج الناتج، ولا يضع حدًا منفصلًا لمدة فيديو المرجع المُدخل. بالنسبة إلى `grok-imagine-video`، تقبل reference-to-video حتى 7 مراجع صور (`reference_images` أو `image_urls`) ويكون `duration` بحد أقصى 10 ثوانٍ. لا تجمع مراجع الصور مع إدخالات الإطار الأول `image_url` / `image`. يدعم `grok-imagine-video-1.5-preview` image-to-video فقط.
...
إذا كان النموذج يحتاج إلى صورة للعنصر وفيديو مرجعي للحركة معًا، فاستخدم `operation=motion-control`. يقوم AI Sonar بتطبيع الشكل العام `image_url` + `video_url` إلى تنسيق motion-control لذلك النموذج.
