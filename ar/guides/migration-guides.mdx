---
title: "أدلة الهجرة"
description: "نقل أحمال العمل من OpenAI وAnthropic وGemini ووسائط الإعلام إلى AI Sonar مع تغييرات صغيرة وآمنة للإنتاج."
---

AI Sonar يدعم تنسيقات متعددة: يمكنك الاحتفاظ بعملاء متوافقين مع OpenAI، واستدعاءات رسائل أصلية من Anthropic، واستدعاءات REST أصلية من Gemini، ونقاط نهاية الوسائط في أشكالها الطبيعية. الهجرة الأكثر أمانًا هي عدم ترجمة كل عبء عمل إلى تنسيق عالمي واحد. اختر المسار الذي يمتلك السلوك الذي يحتاجه تطبيقك.

## تخطيط المسار

| عبء العمل الحالي | عنوان URL الأساسي لـ AI Sonar | نقطة النهاية الرئيسية | ملاحظة الهجرة |
|-------------------|-------------------|------------------|----------------|
| إكمالات دردشة OpenAI | `https://api.aisonar.dev/v1` | `/chat/completions` | أقل تغيير لدردشة OpenAI المتوافقة واستدعاء الوظائف |
| استجابات OpenAI | `https://api.aisonar.dev/v1` | `/responses` | استخدمه عندما يعتمد تطبيقك على مدخلات أو أدوات أو معالجة مخرجات محددة للاستجابات |
| SDK من Anthropic | `https://api.aisonar.dev` | `/v1/messages` | لا تضف `/v1` إلى عنوان URL الأساسي لـ SDK |
| Gemini REST | `https://api.aisonar.dev` | `/v1beta/models/:model:generateContent` | احتفظ بالحقول الأصلية من Gemini على مسار Gemini |
...
| الإدارة والفوترة | `https://api.aisonar.dev/v1` | `/management/...` | استخدم رموز الإدارة للاستخدام من جانب الخادم وتسوية الفواتير |
...
2. اقرأ `aisonar.public_contract_summary` في استجابات القائمة و`aisonar.public_contract` الكامل حيثما كان متاحًا.
...
- لا تقارن الفوترة مع معرفات مهام المزود. استخدم سجلات استخدام AI Sonar.
