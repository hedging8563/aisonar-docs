...
الحقول الأساسية (`message`, `type`, `code`, `param`) موجودة دائمًا. حقول التلميح (`did_you_mean`, `suggestions`, `hint`, `retryable`, `retry_after`, `balance_usd`, `estimated_cost_usd`) هي امتدادات اختيارية لتصحيح ذاتي لوكلاء الذكاء الاصطناعي. راجع [دليل Agent-First API](/guides/agent-first-api) للتفاصيل.

تستخدم نقاط النهاية المتوافقة مع OpenAI أنواع أخطاء بوابة AI Sonar المستقرة. تستخدم نقاط النهاية المتوافقة مع Anthropic وGemini عائلات أخطائها وشكل الاستجابات الخاصة بها.
...
لا تميز المسارات العامة بين حالات الخطأ الإملائي أو المخفي أو المؤجل أو غير العام في جسم الاستجابة. إذا لم يكن النموذج متوفرًا حاليًا عبر العقدة العامة، تعيد AI Sonar `model_not_found`.
