...
<ParamField body="material_asset_id" type="string">
  AI Sonar Seedance material asset ID returned by [Create Material Asset](/api-reference/video/create-material-asset). Use it with Seedance 2.0 generation models after the asset is `ACTIVE`.
</ParamField>

<ParamField body="material_asset_ids" type="array">
  Multiple AI Sonar Seedance material asset IDs. They share the same Seedance image-reference limit as `reference_images`; for Seedance 2.0, send at most 9 image references in total.
</ParamField>
...
<ParamField body="output_audio" type="boolean">
  Model-dependent audio output toggle. In AI Sonar, Veo 3 family requests default to `true` when this field is omitted. `kling-3.0-video` accepts this selector for non-element-reference requests and uses the compatible sound control for that model; omitted Kling requests default to silent output. Do not combine `output_audio=true` with `kling_elements`. Other video models use their documented defaults. The camelCase alias `outputAudio` is accepted for compatibility.
</ParamField>
...
Use `operation=reference-to-video` when the model supports dedicated reference conditioning. In AI Sonar requests, image references use `reference_images`, while multimodal reference videos and audios use `video_urls` and `audio_urls`. For `seedance-2.0` and `seedance-2.0-fast`, AI Sonar currently supports up to 9 reference images plus up to 3 reference videos and 3 reference audios. For model selection, 4K boundaries, and Mini notes, see the [Seedance 2.0 video models guide](/guides/seedance-2-video). `duration` controls generated output length only; it does not set a separate limit for reference video input duration. For `grok-imagine-video`, reference-to-video accepts up to 7 image references (`reference_images` or `image_urls`) and `duration` is capped at 10 seconds. Do not combine reference images with `image_url` / `image` first-frame inputs. `grok-imagine-video-1.5-preview` is image-to-video only.
