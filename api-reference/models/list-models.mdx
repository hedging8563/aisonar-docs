...
<ResponseField name="data" type="array">
  ...
  - `tokenlab.pricing_unit` (string or `null`): Public pricing unit when available
  - `tokenlab.capability_flags` (object or `null`): Public capability summary
  - `tokenlab.supported_operations` (array or `null`): Public non-chat operations for the model
  - `tokenlab.pricing_provenance` (object or `null`): Public pricing source metadata
  - `tokenlab.public_contract_summary` (object or `null`): Lightweight non-chat discovery summary with only `public_operations` and `request_endpoint`
  - `tokenlab.agent_preferences` (object): Only returned when `recommended_for` is present
