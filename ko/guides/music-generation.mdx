...
현재 공개 예제는 음악 생성을 위해 `suno_music`을 사용하고 `chirp-v4` 같은 공식 Suno 모델 버전을 `mv`에 포함합니다. 가사 전용 흐름의 경우, 가사 생성을 문서화한 모델과 함께 `action: "LYRICS"`를 전송하고 `mv`는 생략하세요. 모델 ID는 공개 AI Sonar ID로 취급하며, 제공자 특정 필드가 모델 세부정보 필드라는 보장은 아닙니다.
...
프롬프트, 제목 및 태그는 사용자에게 보이고 안전하게 저장할 수 있도록 유지하십시오. API 키, 개인 URL 또는 비공개 진단 정보를 어떤 프롬프트 필드에도 넣지 마십시오.
...
| 지원되지 않는 매개변수 | 필드가 모델 모델 세부정보에 없음 | 제공자 특정 필드를 제거하거나 이를 문서화한 모델을 선택 |
