---
title: "비동기 작업 및 폴링"
description: "비동기 미디어 작업을 생성하고, 공개 AI Sonar 작업 상태를 폴링하며, 공급자 특정 ID 없이 최종 결과를 조정합니다."
---

많은 미디어 엔드포인트는 비동기입니다. 생성 요청은 작업을 시작하고 공개 AI Sonar 작업 ID를 반환합니다. 애플리케이션은 해당 작업이 최종 상태에 도달할 때까지 폴링합니다. 고객 워크플로우를 업스트림 작업 URL, 라우팅 ID 또는 공급자 콜백 동작을 기반으로 구축하지 마십시오.
...
| 필드 | 의미 | 수행할 작업 |
|-------|---------|------------|
| `id` | 공개 AI Sonar 작업 ID | 자신의 작업 기록과 함께 저장합니다 |
...
`DELETE /v1/tasks/{id}`는 의도적으로 좁습니다. 현재 `seedance-1.5-pro`, `seedance-2.0`, 및 `seedance-2.0-fast`와 같은 대기 중인 Volcengine Seedance 비디오 작업을 지원합니다.
