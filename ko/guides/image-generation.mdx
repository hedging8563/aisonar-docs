---
title: "이미지 생성"
description: "올바른 AI Sonar 이미지 엔드포인트를 선택하고, 모델 인식 요청을 전송하며, 동기 또는 비동기 이미지 결과를 안전하게 처리합니다."
---

AI Sonar은 공용 이미지 엔드포인트를 통해 텍스트-투-이미지, 이미지-투-이미지 및 이미지 편집을 지원합니다. 이미지 모델은 하나의 보편적인 매개변수 집합을 공유하지 않으므로, 프로덕션 클라이언트는 먼저 엔드포인트를 선택한 다음 모델을 선택하고, 해당 모델에서 지원하는 필드만 전송해야 합니다.
...
모델 발견으로 시작한 다음 선택한 모델의 AI Sonar 계약을 검사하십시오:
...
비채팅 모델의 경우, 목록 응답에는 `aisonar.public_contract_summary`가 포함될 수 있습니다. 모델 세부 페이지는 더 완전한 `aisonar.public_contract`를 노출할 수 있습니다. 이러한 필드를 사용하여 확인하십시오:
...
- `gpt-image-2` 스타일 요청은 OpenAI와 유사한 `size`, `quality`, 및 편집 필드를 사용합니다. 모델이나 AI Sonar이 자동 기본값을 사용하도록 하려면 선택적 필드를 생략하십시오.
...
- AI Sonar을 호출하기 전에 사용자 입력을 검증하십시오: 프롬프트 길이, 이미지 수, URL 도달 가능성, 및 파일 유형.
