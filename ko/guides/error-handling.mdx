...
기본 필드 (`message`, `type`, `code`, `param`)는 항상 존재합니다. 힌트 필드 (`did_you_mean`, `suggestions`, `hint`, `retryable`, `retry_after`, `balance_usd`, `estimated_cost_usd`)는 AI 에이전트의 자체 수정(self-correction)을 위한 선택적 확장입니다. 자세한 내용은 [Agent-First API 가이드](/guides/agent-first-api)를 참조하세요.

OpenAI 호환 endpoints는 AI Sonar의 안정적인 게이트웨이 오류 유형을 사용합니다. Anthropic-compatible 및 Gemini-compatible endpoints는 자체 네이티브 오류 계열 및 응답 형태를 사용합니다.
...
| `model_not_found` | 요청한 모델이 현재 공개 계약에서 사용 불가 |
...
공개 라우트는 응답 본문에서 오타, 비공개(hidden), 보류(deferred), 비공개 상태를 구분하지 않습니다. 모델이 현재 공개 계약을 통해 사용 불가능한 경우, AI Sonar는 `model_not_found`를 반환합니다.
