...
AI Sonar 공개 비디오 계약은 다음 작업 값을 허용합니다. 지원 여부는 모델별로 다르며 공급자가 기능을 추가하거나 종료하면 변경됩니다. 특수 작업에 의존하기 전에 선택한 모델 계약을 확인하세요.
...
- **T2V (Text-to-Video)**: 텍스트 프롬프트에서 비디오를 생성합니다.
- **I2V (Image-to-Video)**: 시작 이미지를 기반으로 비디오를 생성합니다. 가장 넓은 호환성을 위해 `image_url`을 권장합니다.
- **Reference**: `reference_images`로 하나 이상의 참조 이미지를 전달해 생성을 조건화합니다. 일부 모델은 `video_urls`의 참조 비디오와 `audio_urls`의 참조 오디오도 허용합니다.
- **Start-End**: `start_image`와 `end_image`로 첫 프레임과 마지막 프레임을 제어합니다.
- **V2V (Video-to-Video)**: 기존 비디오, 생성된 작업, 또는 공급자별 파생 흐름을 소스로 사용합니다.
- **Motion**: 피사체 이미지와 모션 참조 비디오를 함께 사용합니다.
- **Audio-to-Video**: 오디오 조건이 있는 모델 흐름으로 비디오를 생성합니다.
- **Video Extension**: 기존 비디오 생성 작업을 계속하거나 확장합니다.

## 모델 검색

비디오 모델 가용성은 자주 바뀝니다. 모델을 선택하기 전에 현재 공개 추천 목록을 가져오세요.
...
`aisonar.capabilities`, `aisonar.supported_operations`, `aisonar.public_contract_summary`, `aisonar.public_contract`를 진실 소스로 사용하세요. 아래 예시는 워크플로 패턴이며 전체 모델 인벤토리가 아닙니다.
...
| `video_url` | string | 비디오 URL 기반 `video-to-video` 흐름과 `motion-control`에 필요합니다. 일부 파생 흐름은 대신 `task_id`를 사용합니다. |
...
<CardGroup cols={2}>
  <Card title="최고 화질" icon="crown">
    속도보다 품질이 중요하다면 **veo3.1**, **kling-video-o1-pro**, **viduq3-pro** 가 좋은 선택입니다.
  </Card>
  ...
  <Card title="비디오에서 비디오로" icon="film">
    먼저 `GET /v1/models?recommended_for=video`를 호출하세요. 현재 V2V 스타일 예시는 **grok-imagine-video**, **seedance-2.0**, **veo3.1**, **kling-video-o1-pro / std**입니다.
  </Card>
</CardGroup>
