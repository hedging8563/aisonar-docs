...
<ResponseField name="data" type="array">
  ...
  - `tokenlab.pricing_unit` (string 또는 `null`): 공개 가격 단위
  - `tokenlab.capability_flags` (object 또는 `null`): 공개 기능 요약
  - `tokenlab.supported_operations` (array 또는 `null`): 공개 비채팅 작업 목록
  - `tokenlab.pricing_provenance` (object 또는 `null`): 공개 가격 출처 메타데이터
  - `tokenlab.public_contract_summary` (object 또는 `null`): `public_operations` 와 `request_endpoint` 만 포함하는 간단한 비채팅 탐색 요약
  - `tokenlab.agent_preferences` (object): `recommended_for` 가 있을 때만 반환
  - `tokenlab.lifecycle`(object): 수명 주기 단계, 출시/지원 중단 날짜, 대체 모델, latest 배지 기준
