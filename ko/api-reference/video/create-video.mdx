...
모델 상세 응답의 `aisonar.capabilities`, `aisonar.supported_operations`, `aisonar.public_contract_summary`, `aisonar.public_contract` 를 확인하세요. `audio-to-video` 와 `video-extension` 같은 작업은 모델별 기능입니다. 이 페이지의 정적 예시에 의존하지 말고 그곳에서 현재 지원 여부를 확인하세요.
