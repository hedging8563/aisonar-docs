...
<ParamField body="model" type="string" default="veo3.1">
  비디오 모델 ID 입니다. API 기본값은 `veo3.1` 입니다. 현재 공개 비디오 모델 기능은 [비디오 생성 가이드](/ko/guides/video-generation)와 [Models API](/ko/api-reference/models/list-models)에서 확인하세요.
</ParamField>
...
모델 상세 응답의 `tokenlab.capabilities`, `tokenlab.supported_operations`, `tokenlab.public_contract_summary`, `tokenlab.public_contract` 를 확인하세요. `audio-to-video` 와 `video-extension` 같은 작업은 모델별 기능입니다. 이 페이지의 정적 예시에 의존하지 말고 그곳에서 현재 지원 여부를 확인하세요.
