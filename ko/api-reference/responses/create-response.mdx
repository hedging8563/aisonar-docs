...
The Responses API는 OpenAI의 최신 상태 유지 대화 API입니다. AI Sonar는 호환 모델에 대해 고급 선택 경로로 이 형식을 지원합니다; Responses 고유의 동작이 명시적으로 필요한 경우가 아니면 기본적으로 OpenAI 호환 경로인 `POST /v1/chat/completions`를 사용하세요.

## 요청 본문

<ParamField body="model" type="string" required>
  사용할 모델의 ID입니다. 사용 가능한 옵션은 [Models](https://aisonar.dev/models)을 참조하세요.
</ParamField>
...
<ParamField body="tools" type="array">
  ...
  기본 이미지 도구 모델을 사용하거나 명시적으로 `model: "gpt-image-2"`를 설정한 hosted `image_generation` 도구의 경우, GPT Image 2가 이미지 입력을 이미 high fidelity로 처리하므로 AI Sonar는 지원되지 않는 `input_fidelity`를 upstream으로 전달하기 전에 제거합니다. 이 도구에는 `background: "transparent"`를 보내지 마세요. 출력 의미가 달라지므로 AI Sonar는 이를 조용히 제거하지 않습니다.
</ParamField>
