...
<ResponseField name="data" type="array">
  ...
  - `tokenlab.pricing_unit` (string or `null`): 可用时的公开定价单位
  - `tokenlab.capability_flags` (object or `null`): 公开能力摘要
  - `tokenlab.supported_operations` (array or `null`): 模型公开支持的非聊天操作
  - `tokenlab.pricing_provenance` (object or `null`): 公开定价来源元数据
  - `tokenlab.public_contract_summary` (object or `null`): 轻量级非聊天发现摘要，仅包含 `public_operations` 和 `request_endpoint`
