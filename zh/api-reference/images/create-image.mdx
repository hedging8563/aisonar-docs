...
`gpt-image-2` 是按 token 计费的 GPT Image 模型。AI Sonar 按 OpenAI 官方 usage 明细结算文本输入、图片输入、已上报的缓存输入以及图片输出 token；它不是固定每张图片计费的模型。

对于 `gpt-image-2` 图像生成，公共参数支持 `prompt`、`n`、`size`、`quality`、`response_format`、`async`、`background`、`output_format`、`output_compression` 或 `compression`、`moderation`、`partial_images` 和 `user`。不传 `size` 或 `quality` 时 AI Sonar 会使用 `auto`；自定义 `size` 必须使用下方说明的弹性 `WIDTHxHEIGHT` 契约。
...
- `nano-banana-2` 在当前 AI Sonar 契约中仅文生图支持 `aspect_ratio` 和 `resolution`（`1k`、`2k`、`4k`）。
...
- Nano Banana 图生图请求中，`nano-banana-pro` 可以发送 `resolution`（`1k`、`2k`、`4k`）；`nano-banana-edit` 必须省略该参数。`nano-banana` 和 `nano-banana-2` 在当前公开契约中是文生图模型。
...
<ParamField body="size" type="string" default="1024x1024">
  ...
  对于 `gpt-image-2`，`size` 接受 `auto` 或 `WIDTHxHEIGHT`。自定义宽高必须都是 16 的倍数，最长边不超过 `3840px`，长边/短边比例不超过 `3:1`，总像素必须在 `655,360` 到 `8,294,400` 之间。`aspect_ratio` 和 `resolution` 目前不属于 AI Sonar 对 `gpt-image-2` 的公共契约。
...
<ResponseField name="data" type="array">
  ...
  - `revised_prompt` (string): 上游模型返回的可选提示词改写结果（仅在提供者返回时出现）
</ResponseField>
