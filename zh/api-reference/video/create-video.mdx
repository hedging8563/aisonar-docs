...
公开视频模型库存和支持的操作会持续变化。接入某个模型特定流程前，请以 [Models API](/zh/api-reference/models/list-models) 作为真值来源：
...
请读取单模型详情响应中的 `tokenlab.capabilities`、`tokenlab.supported_operations`、`tokenlab.public_contract_summary` 和 `tokenlab.public_contract`。`audio-to-video`、`video-extension` 等操作属于模型特定能力；请在那里确认实时可用性，不要依赖本页中的静态示例。
