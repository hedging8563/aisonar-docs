...
请读取单模型详情响应中的 `aisonar.capabilities`、`aisonar.supported_operations`、`aisonar.public_contract_summary` 和 `aisonar.public_contract`。`audio-to-video`、`video-extension` 等操作属于模型特定能力；请在那里确认实时可用性，不要依赖本页中的静态示例。
