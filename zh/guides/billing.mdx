...
实时价格会随供应商、路由和模型契约变化。请以 Dashboard、模型页、`GET /v1/models/:model/pricing` 和 [Pricing API](/api-reference/pricing/get-pricing) 为准。
...
图像、视频、音乐、3D、音频和 world 生成模型可能按请求、图片、秒/分钟、任务或供应商特定用量计费。生产使用前请检查所选模型契约。
...
如果任务已经进入终态，但 dashboard 中没有看到对应结算或退款，请联系  寻求帮助。
