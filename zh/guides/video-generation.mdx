...
AI Sonar 公开视频契约接受以下操作值。支持情况与具体模型有关，并会随着供应商新增或下线能力而变化；依赖某个专用操作前，请先读取所选模型契约。
...
请把 `aisonar.capabilities`、`aisonar.supported_operations`、`aisonar.public_contract_summary` 和 `aisonar.public_contract` 作为真值来源。下面的示例是工作流模式，不是完整模型库存。
