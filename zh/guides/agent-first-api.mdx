...
AI Sonar 的 Agent-First API 会在错误响应中加入结构化提示，AI Agent 可以立即解析并采取行动 —— 无需网络搜索、无需查阅文档、无需猜测。

每个错误响应的标准 `error` 对象中都包含可选字段，例如 `did_you_mean`、`suggestions`、`hint`、`retryable` 和 `retry_after`。这些字段向后兼容 —— 不使用它们的客户端不会有任何差异。
...
<Note>
与 OpenAI 兼容的端点使用 AI Sonar 的稳定公共错误类型，例如 `rate_limit_exceeded`、`upstream_error` 和 `all_channels_failed`。与 Anthropic 兼容和与 Gemini 兼容的端点使用它们各自的原生响应格式。
</Note>
...
```bash
GET https://api.aisonar.dev/v1/models?category=chat          # 仅聊天模型
GET https://api.aisonar.dev/v1/models?category=image         # 图片生成模型
GET https://api.aisonar.dev/v1/models?tag=coding&category=chat  # 面向 coding 的聊天模型
```
