...
<ParamField body="material_asset_id" type="string">
  ID de material AI Sonar Seedance retornado por [Criar material](/pt/api-reference/video/create-material-asset). Use com modelos Seedance 2.0 depois que o material estiver `ACTIVE`.
</ParamField>

<ParamField body="material_asset_ids" type="array">
  Vários IDs de material AI Sonar Seedance. Eles compartilham o mesmo limite de referências de imagem de `reference_images`; no Seedance 2.0, envie no máximo 9 referências de imagem no total.
</ParamField>
...
Quando um modelo precisa tanto de uma imagem do sujeito quanto de um vídeo de referência de movimento, use `operation=motion-control`. A AI Sonar normaliza a forma pública `image_url` + `video_url` para o formato de solicitação esperado pelo modelo.
