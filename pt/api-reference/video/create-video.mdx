...
<ParamField body="model" type="string" default="veo3.1">
  ID do modelo de vídeo. O padrão da API é `veo3.1`. Consulte o [guia de geração de vídeo](/pt/guides/video-generation) e a [Models API](/pt/api-reference/models/list-models) para as capacidades públicas atuais de vídeo.
</ParamField>
...
O inventário público de vídeo e as operações compatíveis mudam com o tempo. Use a [Models API](/pt/api-reference/models/list-models) como fonte da verdade antes de integrar um fluxo específico de modelo:
...
Leia `tokenlab.capabilities`, `tokenlab.supported_operations`, `tokenlab.public_contract_summary` e `tokenlab.public_contract` na resposta de detalhe do modelo. Operações como `audio-to-video` e `video-extension` são específicas de cada modelo; confirme a disponibilidade atual ali, em vez de depender de exemplos estáticos desta página.
