...
O cancelamento é suportado para tarefas de vídeo Seedance em fila quando o provedor selecionado para essa tarefa oferece cancelamento. Os modelos compatíveis são `seedance-1.5-pro`, `seedance-2.0` e `seedance-2.0-fast`. Tarefas que já estão em `processing`/running não podem ser canceladas. Tarefas concluídas, com falha, expiradas ou não suportadas continuam consultáveis para status e conciliação de cobrança.
