...
description: 'Referência completa da API AI Sonar'
---
...
Obtenha sua chave de API a partir do [Dashboard](https://console.aisonar.dev).
...
<Info>
Use `/v1/realtime` para solicitações de upgrade WebSocket. Um `GET /v1/realtime` comum retorna metadados do endpoint para clientes que não conseguem inspecionar rotas WebSocket diretamente.
</Info>
...
| `channel.channelType` | `PLATFORM` (AI Sonar) ou `PRIVATE` (BYOK) |
