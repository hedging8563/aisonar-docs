...
<ResponseField name="data" type="array">
  ...
  - `tokenlab.pricing_unit` (string or `null`): Unidade pública de preço quando disponível
  - `tokenlab.capability_flags` (object or `null`): Resumo público de capacidades
  - `tokenlab.supported_operations` (array or `null`): Operações públicas não chat compatíveis com o modelo
  - `tokenlab.pricing_provenance` (object or `null`): Metadados públicos de procedência de preços
  - `tokenlab.public_contract_summary` (object or `null`): Resumo leve de descoberta não chat com apenas `public_operations` e `request_endpoint`
