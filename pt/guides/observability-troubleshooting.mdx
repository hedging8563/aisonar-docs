...
Uma boa observabilidade do AI Sonar começa com identificadores públicos. Seus logs devem ajudá-lo a responder "o que o usuário solicitou, qual tarefa pública o AI Sonar criou e como foi cobrado?" sem expor detalhes sensíveis do provedor ou dados sensíveis do usuário.
...
`GET /v1/management/api-keys/{keyId}/usage` pode filtrar por `scene`, `accessChannel`, `model`, `modelVendor`, `startDate` e `endDate`. Use esses registros em vez de raspar páginas de dashboard ou depender de IDs de tarefas do provedor.
...
Não inclua chaves de API, tokens de gerenciamento, mídia privada, prompts completos, URLs do provedor, IDs de canal ou identificadores privados de diagnóstico, a menos que o suporte do AI Sonar peça explicitamente um exemplo redigido.
