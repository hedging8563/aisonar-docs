...
Os exemplos públicos atuais usam `suno_music` para geração de música e incluem `mv` com uma versão oficial do modelo Suno, como `chirp-v4`. Para fluxos apenas de letras, envie `action: "LYRICS"` com um modelo cujo detalhes do modelo documente a geração de letras e omita `mv`. Trate os IDs de modelo como IDs públicos do AI Sonar, não como uma garantia de que campos específicos do provedor são campos de detalhes do modelo.
...
Mantenha prompts, títulos e tags visíveis para o usuário e seguros para armazenar. Não coloque chaves de API, URLs privadas ou informações privadas de diagnóstico em nenhum campo de prompt.
...
| Parâmetro não suportado | Campo não está nos detalhes do modelo do modelo | Remova campos específicos do provedor ou escolha um modelo que os documente |
