...
A geração 3D é assíncrona. `POST /v1/3d/generations` cria uma tarefa AI Sonar; respostas de status concluído retornam ativos de modelo para download, como `model_url` e, quando disponíveis, URLs específicas de formato.
