...
Comece com a descoberta do modelo e, em seguida, inspecione o contrato do AI Sonar do modelo selecionado:
...
Para modelos não de chat, a resposta da lista pode incluir `aisonar.public_contract_summary`. As páginas de detalhes do modelo podem expor o `aisonar.public_contract` mais completo. Use esses campos para confirmar:
...
- Solicitações estilo `gpt-image-2` usam campos de `size`, `quality` e edição semelhantes aos da OpenAI. Deixe de fora os campos opcionais quando você quiser que o modelo ou o AI Sonar usem padrões automáticos.
...
- Valide as entradas do usuário antes de chamar o AI Sonar: comprimento do prompt, contagem de imagens, acessibilidade da URL e tipo de arquivo.
