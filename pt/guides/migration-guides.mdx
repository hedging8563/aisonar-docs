...
description: "Mova cargas de trabalho OpenAI, Anthropic, Gemini e de mídia para o AI Sonar com pequenas mudanças seguras para produção."
---

AI Sonar é multi-formato: você pode manter clientes compatíveis com OpenAI, chamadas de Messages nativas da Anthropic, chamadas REST nativas da Gemini e endpoints de mídia em suas formas naturais. A migração mais segura não é traduzir cada carga de trabalho em um formato universal. Escolha a rota que possui o comportamento que sua aplicação precisa.

## Mapeamento de Rotas

| Carga de trabalho existente | URL base do AI Sonar | Endpoint principal | Nota de migração |
|-----------------------------|----------------------|--------------------|------------------|
| OpenAI Chat Completions     | `https://api.aisonar.dev/v1` | `/chat/completions` | Menor mudança para chat compatível com OpenAI e chamadas de função |
| OpenAI Responses            | `https://api.aisonar.dev/v1` | `/responses` | Use quando seu aplicativo depende de entrada, ferramentas ou manipulação de saída específicas de Responses |
| Anthropic SDK               | `https://api.aisonar.dev` | `/v1/messages` | Não acrescente `/v1` à URL base do SDK |
| Gemini REST                 | `https://api.aisonar.dev` | `/v1beta/models/:model:generateContent` | Mantenha campos nativos da Gemini na rota da Gemini |
...
| Gestão e faturamento        | `https://api.aisonar.dev/v1` | `/management/...` | Use tokens de gestão para uso do lado do servidor e reconciliação de faturamento |
...
2. Leia `aisonar.public_contract_summary` nas respostas da lista e o `aisonar.public_contract` completo onde disponível.
...
- Não compare faturamento com IDs de tarefa do provedor. Use registros de uso do AI Sonar.
