...
O contrato público de vídeo da AI Sonar aceita estes valores de operação. O suporte depende do modelo e muda quando provedores adicionam ou removem capacidades; verifique o contrato do modelo selecionado antes de depender de uma operação especializada.
...
Use `aisonar.capabilities`, `aisonar.supported_operations`, `aisonar.public_contract_summary` e `aisonar.public_contract` como fonte de verdade. Os exemplos abaixo são padrões de workflow, não um inventário exaustivo.
