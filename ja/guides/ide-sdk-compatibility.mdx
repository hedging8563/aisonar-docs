...
- **ベストエフォート（ベストエフォート）** は、統合は機能する可能性があるが、このカスタムゲートウェイワークフローが公式連携ではないことを意味します。
...
| **Gemini CLI** | ベストエフォート / 実験的 | Gemini | カスタム AI Sonar base URL フローは Gemini CLI の公式連携ではありません |
...
<AccordionGroup>
  <Accordion title="不明なパラメータには passthrough を使う">
    未知のパラメータは、選択された公開ルートとモデルが対応している場合にのみ転送されます。
  </Accordion>
