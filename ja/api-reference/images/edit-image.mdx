...
互換性メモ: `gpt-image-2` リクエストで `input_fidelity` が送信された場合、GPT Image 2 は画像入力を自動的に高忠実度で処理するため、AI Sonar はこのフィールドを削除します。
</Note>
