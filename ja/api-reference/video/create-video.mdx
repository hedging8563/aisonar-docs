...
<ParamField body="material_asset_id" type="string">
  [素材を作成](/ja/api-reference/video/create-material-asset) が返す AI Sonar Seedance 素材 ID です。素材が `ACTIVE` になった後、Seedance 2.0 生成モデルで使えます。
</ParamField>

<ParamField body="material_asset_ids" type="array">
  複数の AI Sonar Seedance 素材 ID です。`reference_images` と同じ Seedance の画像参照数制限を共有し、Seedance 2.0 では合計最大 9 件です。
</ParamField>
...
<ParamField body="output_audio" type="boolean">
  モデル依存の音声出力トグルです。AI Sonar では Veo 3 系列は省略時に `true` として扱われます。`kling-3.0-video` は要素参照を使わないリクエストでこの selector を受け付け、そのモデルの互換 sound 制御を使用します。Kling リクエストで省略した場合は無音が既定です。`output_audio=true` と `kling_elements` は併用できません。他の動画モデルは各ドキュメントの既定値を使用します。camelCase の別名 `outputAudio` も受け付けます。
</ParamField>
...
主体画像とモーション参照動画の両方を必要とするモデルでは `operation=motion-control` を使用します。AI Sonar は公開リクエスト形の `image_url` と `video_url` をモデルが必要とするリクエスト形式に正規化します。
