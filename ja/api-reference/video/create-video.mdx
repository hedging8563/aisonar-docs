...
<ParamField body="model" type="string" default="veo3.1">
  動画モデル ID。API のデフォルトは `veo3.1` です。現在の公開動画モデルの能力は [動画生成ガイド](/ja/guides/video-generation) と [Models API](/ja/api-reference/models/list-models) で確認してください。
</ParamField>
...
モデル詳細レスポンスの `aisonar.capabilities`、`aisonar.supported_operations`、`aisonar.public_contract_summary`、`aisonar.public_contract` を参照してください。`audio-to-video` や `video-extension` などの操作はモデル固有です。本ページの静的な例ではなく、そこで現在の可用性を確認してください。
