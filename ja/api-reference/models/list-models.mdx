...
<ResponseField name="data" type="array">
  ...
  - `tokenlab.pricing_unit` (string or `null`): 利用可能な場合の公開料金単位
  - `tokenlab.capability_flags` (object or `null`): 公開されている機能サマリー
  - `tokenlab.supported_operations` (array or `null`): モデルが公開サポートする非チャット操作
  - `tokenlab.pricing_provenance` (object or `null`): 公開料金の出典メタデータ
  - `tokenlab.public_contract_summary` (object or `null`): `public_operations` と `request_endpoint` のみを含む軽量な非チャット発見用サマリー
