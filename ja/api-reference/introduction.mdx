---
title: 'APIリファレンス'
description: 'AI Sonar API の完全なリファレンス'
---

## 概要

AI Sonar は **native-first かつ OpenAI 互換** です。ネイティブな挙動が必要な場合は、Anthropic では `POST /v1/messages` のような provider-native のルートを、Gemini では `/v1beta/models/...:generateContent` を使用してください。既存の OpenAI 形式の SDK やツールを移行している場合は、OpenAI 互換 な `/v1` エンドポイントを使用してください。`POST /v1/responses` は、Responses 固有の動作向けの上級者向けオプション経路のままです。
...
APIキーは[ダッシュボード](https://console.aisonar.dev)から取得してください。
...
<Info>
`/v1/realtime` は WebSocket アップグレード要求に使用します。通常の `GET /v1/realtime` は、WebSocket ルートを直接確認できないクライアント向けにエンドポイントメタデータを返します。
</Info>
...
| `channel.channelType` | `PLATFORM`（AI Sonar）または `PRIVATE`（BYOK） |
