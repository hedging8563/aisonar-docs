---
title: "Gemini CLI"
description: "AI Sonar API を使用するように Google Gemini CLI を設定する"
---
...
Google Gemini CLI は、Gemini モデルとやり取りするためのコマンドラインツールです。AI Sonar は互換性のあるエンドポイントを提供しており、Gemini CLI を使用して 300 以上のモデルにアクセスできます。


<Warning>
これは実験的な互換構成です。Gemini CLI は現在、AI Sonar のようなカスタム Base URL ワークフローを安定した形で公式には文書化していません。サポートされた AI Sonar のターミナル運用が必要なら、Codex CLI、Claude Code、OpenCode を優先してください。
</Warning>
...
1. [AI Sonar ダッシュボード](https://console.aisonar.dev)にログインします
2. [API キー](https://console.aisonar.dev/api)に移動します
...
<AccordionGroup>
  <Accordion title="接続エラー">
    - `GOOGLE_GEMINI_BASE_URL` が `https://api.aisonar.dev` に設定されていることを確認してください
  ...
  <Accordion title="認証失敗">
    ...
    - AI Sonar ダッシュボードでキーが有効であることを確認してください
  </Accordion>

  <Accordion title="モデルが利用不可">
    - [aisonar.dev/en/models](https://aisonar.dev/models) で Gemini モデルの利用可能性を確認してください
