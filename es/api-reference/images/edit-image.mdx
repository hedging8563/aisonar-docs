...
Nota de compatibilidad: si se envía `input_fidelity` con `gpt-image-2`, AI Sonar lo elimina porque GPT Image 2 ya procesa las entradas de imagen con alta fidelidad.
</Note>
...
URLs de imagen remotas: cuando se necesita entrada multipart, AI Sonar descarga JSON `image_url`, `image_urls` o `images[].image_url` y envía los bytes como partes multipart `image`. Las URLs deben ser públicas `http`/`https`, sin credenciales incrustadas ni fragmentos, y no deben resolver a localhost ni a rangos IP privados o reservados; cada redirección se valida de nuevo. El contenido descargado debe ser una imagen PNG, JPEG o WebP real. Límites: `50MB` por imagen, `200MB` en total para imágenes descargadas por URL en una solicitud, timeout de `10s` y hasta `3` redirecciones.
...
<ParamField body="response_format" type="string" default="url">
  ...
  Para solicitudes `gpt-image-2` de Azure Official o compatibles con Azure, AI Sonar recibe los datos de imagen como `b64_json`. Para solicitudes `url`, AI Sonar sube cada imagen al CDN y devuelve `data[].url`. Si el almacenamiento CDN no está disponible o la subida falla, la solicitud falla en lugar de convertirse en una respuesta Base64. Para `b64_json`, devuelve el Base64 sin procesar.
</ParamField>
