...
<ParamField body="model" type="string" default="veo3.1">
  ID del modelo de video. Usa los IDs de modelo que muestra AI Sonar como `veo3.1`, `wan-2.7`, `happyhorse-1.0`, `viduq3`, `pixverse-v6` o `kling-3.0-video`; elige text-to-video, image-to-video, reference-to-video u otras variantes con `operation`. Consulta la [guia de video](/es/guides/video-generation) y [Models API](/es/api-reference/models/list-models).
</ParamField>
...
<ParamField body="operation" type="string">
  Operación de video que se va a ejecutar. Los valores admitidos son `text-to-video`, `image-to-video`, `reference-to-video`, `start-end-to-video`, `video-to-video`, `video-extension`, `audio-to-video` y `motion-control`. AI Sonar puede inferir la operación a partir de las entradas, pero en producción se recomienda enviarla de forma explícita.
</ParamField>
...
<ParamField body="material_asset_id" type="string">
  ID de recurso AI Sonar Seedance devuelto por [Crear recurso](/es/api-reference/video/create-material-asset). Úsalo con modelos Seedance 2.0 cuando el recurso esté `ACTIVE`.
</ParamField>

<ParamField body="material_asset_ids" type="array">
  Varios ID de recursos AI Sonar Seedance. Comparten el mismo límite de referencias de imagen que `reference_images`; en Seedance 2.0 envía como máximo 9 referencias de imagen en total.
</ParamField>
...
<ParamField body="output_audio" type="boolean">
  Selector de salida de audio dependiente del modelo. En AI Sonar, las solicitudes de la familia Veo 3 usan `true` por defecto cuando se omite este campo. `kling-3.0-video` acepta este selector para solicitudes sin referencias de elementos y lo asigna al control de sonido compatible de ese modelo; las solicitudes Kling omitidas son silenciosas por defecto. No combines `output_audio=true` con `kling_elements`. Otros modelos de video usan sus valores predeterminados documentados. También se acepta el alias camelCase `outputAudio` por compatibilidad.
</ParamField>
...
Cuando un modelo necesita tanto una imagen del sujeto como un video de referencia de movimiento, usa `operation=motion-control`. AI Sonar normaliza la forma pública `image_url` + `video_url` al formato motion-control de ese modelo.
