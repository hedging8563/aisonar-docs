...
<ParamField body="model" type="string" default="veo3.1">
  ID del modelo de video. El valor predeterminado de la API es `veo3.1`. Consulta la [guía de generación de video](/es/guides/video-generation) y la [Models API](/es/api-reference/models/list-models) para ver las capacidades públicas actuales de video.
</ParamField>
...
Lee `aisonar.capabilities`, `aisonar.supported_operations`, `aisonar.public_contract_summary` y `aisonar.public_contract` en la respuesta de detalle del modelo. Operaciones como `audio-to-video` y `video-extension` dependen del modelo; confirma allí la disponibilidad actual en lugar de depender de ejemplos estáticos de esta página.
