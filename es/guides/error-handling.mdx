...
Los campos base (`message`, `type`, `code`, `param`) siempre están presentes. Los campos de sugerencias (`did_you_mean`, `suggestions`, `hint`, `retryable`, `retry_after`, `balance_usd`, `estimated_cost_usd`) son extensiones opcionales para la autocorrección de agentes de IA. Consulta la [Guía Agent-First API](/guides/agent-first-api) para más detalles.

Los endpoints compatibles con OpenAI usan los tipos de error del gateway estable de AI Sonar. Los endpoints compatibles con Anthropic y Gemini usan sus propias familias de errores y formatos de respuesta nativos.
...
Las rutas públicas no distinguen en el cuerpo de la respuesta entre errores tipográficos, modelos ocultos, diferidos o no públicos. Si un modelo no está disponible actualmente a través dlos detalhes do modelo, AI Sonar devuelve `model_not_found`.
