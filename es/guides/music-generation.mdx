...
Los ejemplos públicos actuales utilizan `suno_music` para la generación de música e incluyen `mv` con una versión oficial del modelo Suno, como `chirp-v4`. Para flujos solo de letras, envía `action: "LYRICS"` con un modelo cuyo detalhes do modelo documente la generación de letras y omite `mv`. Trata los IDs de modelo como IDs públicos de AI Sonar, no como una garantía de que los campos específicos del proveedor son campos de detalhes do modelo.
...
Mantén los prompts, títulos y etiquetas visibles para el usuario y seguros para almacenar. No coloques claves de API, URLs privadas ni información privada de diagnóstico en ningún campo de prompt.
...
| Parámetro no soportado | El campo no está en los detalhes do modelo del modelo | Elimina los campos específicos del proveedor o elige un modelo que los documente |
