...
Comience con el descubrimiento del modelo, luego inspeccione el detalles del modelo en AI Sonar del modelo seleccionado:
...
Para modelos que no son de chat, la respuesta de la lista puede incluir `GET /v1/models`. Las páginas de detalhes do modelo pueden exponer el `GET /v1/models/{model}` más completo. Use esos campos para confirmar:
...
- Las solicitudes estilo `gpt-image-2` utilizan campos de `size`, `quality` y edición similares a OpenAI. Deje fuera los campos opcionales cuando desee que el modelo o AI Sonar utilicen valores predeterminados automáticos.
...
- Valide las entradas del usuario antes de llamar a AI Sonar: longitud del aviso, cantidad de imágenes, accesibilidad de URL y tipo de archivo.
...
| Campo no soportado | El campo no está en los detalhes do modelo de ese modelo | Elimine el campo o elija un modelo/endpoint que lo documente |
