...
If you just want to get started quickly, configuring `tokenlab` alone is enough. Add the other providers only when you explicitly need **Responses API**, **Claude native**, **Gemini native**, or **MiniMax native** behavior.

| Provider | OpenClaw `api` | Best for | baseUrl |
|----------|----------------|----------|---------|
| `tokenlab` | `openai-completions` | GPT, DeepSeek, Qwen, and most OpenAI-compatible calls | `https://api.aisonar.dev/v1` |
| `tokenlab-responses` | `openai-responses` | OpenAI Responses workflows that expect `/v1/responses` semantics | `https://api.aisonar.dev/v1` |
| `tokenlab-claude` | `anthropic-messages` | Native Claude Messages API | `https://api.aisonar.dev` |
...
<AccordionGroup>
  ...
  <Accordion title="Forgetting /v1 on tokenlab-responses">
    `openai-responses` maps to AI Sonar's `/v1/responses` path, so `tokenlab-responses` must use `https://api.aisonar.dev/v1`.
  </Accordion>
