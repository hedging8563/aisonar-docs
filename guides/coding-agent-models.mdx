...
For the newer coding-oriented pool, also consider `qwen-long-latest` for very large repositories, `minimax-m3` for large-context agent work, `kimi-k2.7-code-highspeed` for fast coding loops, and `deepseek-v4-pro` / `deepseek-v4-flash`, `glm-5.2`, `step-3.7-flash`, and `mimo-v2.5-pro` for OpenAI-compatible chat routing.
