...
Structured outputs and tool calls are route-specific contracts. AI Sonar forwards several public API formats, so the safest production pattern is to keep each provider-native tool shape on the route that owns it and validate model output in your own application.
...
Do not send provider-native built-in tools through a different route and expect AI Sonar to silently translate them.
...
The stable shared validation for Chat Completions accepts `text` and `json_object`. `json_schema`, `strict`, and provider-specific schema enforcement may exist for some upstream routes or conversion paths, but they are not a blanket promise across every AI Sonar route and model. Verify them against the selected model before relying on them.
...
AI Sonar does not execute your functions. Your application owns the loop:
