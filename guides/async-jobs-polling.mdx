...
description: "Create asynchronous media jobs, poll public AI Sonar task status, and reconcile final results without provider-specific IDs."
---

Many media endpoints are asynchronous. A create request starts work and returns a public AI Sonar task identity; your application polls until that task reaches a terminal status. Do not build customer workflows around upstream task URLs, routing IDs, or provider callback behavior.
...
|-------|---------|------------|
| `id` | Public AI Sonar task ID | Store it with your own job record |
