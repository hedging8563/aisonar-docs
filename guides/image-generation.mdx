...
description: "Choose the right AI Sonar image endpoint, send model-aware requests, and handle synchronous or async image results safely."
---

AI Sonar supports text-to-image, image-to-image, and image editing through public image endpoints. Image models do not share one universal parameter set, so production clients should first choose the endpoint, then choose the model, then send only the fields supported by that model.
...
Start with model discovery, then inspect the selected model's AI Sonar contract:
...
For non-chat models, the list response may include `aisonar.public_contract_summary`. Model detail pages may expose the fuller `aisonar.public_contract`. Use those fields to confirm:
...
- `gpt-image-2` style requests use OpenAI-like `size`, `quality`, and edit fields. Leave optional fields out when you want the model or AI Sonar to use automatic defaults.
...
- Validate user inputs before calling AI Sonar: prompt length, image count, URL reachability, and file type.
