...
OpenAI-compatible endpoints use AI Sonar's stable gateway error types. Anthropic-compatible and Gemini-compatible endpoints use their own native error families and response shapes.
...
| `model_not_found` | Requested model is not available in the current public contract |
...
Public routes do not distinguish typo, hidden, deferred, or non-public model states in the response body. If a model is not currently available through the public contract, AI Sonar returns `model_not_found`.
