...
Live prices can change as providers, routes, and model contracts change. Treat the Dashboard, the Models page, `GET /v1/models/:model/pricing`, and [Pricing API](/api-reference/pricing/get-pricing) as the source of truth.
...
Image, video, music, 3D, audio, and world-generation models may be priced per request, per image, per second/minute, per task, or by provider-specific usage. Check the selected model contract before production usage.

| Family | Examples |
|---|---|
| Image | `flux-pro`, `qwen-image` |
...
If the dashboard does not reflect a completed settlement or refund after terminal status is visible, contact  for assistance.
