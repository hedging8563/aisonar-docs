...
<ParamField body="model" type="string" default="veo3.1">
  Video model ID’si. API varsayılanı `veo3.1` değeridir. Güncel herkese açık video modeli yetenekleri için [video üretim kılavuzuna](/tr/guides/video-generation) ve [Models API](/tr/api-reference/models/list-models) uç noktasına bakın.
</ParamField>
...
Model detay yanıtındaki `tokenlab.capabilities`, `tokenlab.supported_operations`, `tokenlab.public_contract_summary` ve `tokenlab.public_contract` alanlarını okuyun. `audio-to-video` ve `video-extension` gibi işlemler modele özgüdür; güncel kullanılabilirliği bu sayfadaki statik örneklerden değil oradan doğrulayın.
