...
Model detay yanıtındaki `aisonar.capabilities`, `aisonar.supported_operations`, `aisonar.public_contract_summary` ve `aisonar.public_contract` alanlarını okuyun. `audio-to-video` ve `video-extension` gibi işlemler modele özgüdür; güncel kullanılabilirliği bu sayfadaki statik örneklerden değil oradan doğrulayın.
