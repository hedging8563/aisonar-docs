...
<ParamField body="model" type="string" required>
  Kullanılacak modelin ID'si. Mevcut seçenekler için [Models](https://aisonar.dev/models) sayfasına bakın.
</ParamField>
...
<ParamField body="messages" type="array" required>
  ...
  `content` bir dizi olduğunda, AI Sonar uyumlu modeller için yapılandırılmış multimodal blokları destekler:
  - text: `{ "type": "text", "text": "..." }`
  - image: `{ "type": "image_url", "image_url": { "url": "https://..." } }`
  - video: `{ "type": "video_url", "video_url": { "url": "https://..." } }`
  - audio: `{ "type": "audio_url", "audio_url": { "url": "https://..." } }`

  Multimodal üretim trafiği için genel erişimli `https` URL'leri tercih edin. AI Sonar, bu medya bloklarını yönlendirilen fiziksel modelin gerektirdiği sağlayıcıya özgü istek biçimine dönüştürecektir.
</ParamField>
...
```php PHP
<?php
$ch = curl_init('https://api.aisonar.dev/v1/chat/completions');
