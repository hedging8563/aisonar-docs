...
description: "OpenAI, Anthropic, Gemini ve medya iş yüklerini AI Sonar'a küçük, üretim güvenli değişikliklerle taşıyın."
---

AI Sonar çok formatlıdır: OpenAI uyumlu istemcileri, Anthropic yerel Messages çağrılarını, Gemini yerel REST çağrılarını ve medya uç noktalarını doğal şekillerinde tutabilirsiniz. En güvenli göç, her iş yükünü tek bir evrensel formata çevirmemektir. Uygulamanızın ihtiyaç duyduğu davranışı sahiplenen rotayı seçin.

## Rota Haritalama

| Mevcut iş yükü | AI Sonar temel URL'si | Birincil uç nokta | Göç notu |
|-------------------|-------------------|------------------|----------------|
| OpenAI Sohbet Tamamlamaları | `https://api.aisonar.dev/v1` | `/chat/completions` | OpenAI uyumlu sohbet ve fonksiyon çağrıları için en küçük değişiklik |
| OpenAI Yanıtları | `https://api.aisonar.dev/v1` | `/responses` | Uygulamanız Yanıtlar'a özel girdi, araçlar veya çıktı işleme bağımlıysa kullanın |
| Anthropic SDK | `https://api.aisonar.dev` | `/v1/messages` | SDK temel URL'sine `/v1` eklemeyin |
| Gemini REST | `https://api.aisonar.dev` | `/v1beta/models/:model:generateContent` | Gemini rotasında Gemini yerel alanlarını koruyun |
...
| Yönetim ve faturalama | `https://api.aisonar.dev/v1` | `/management/...` | Sunucu tarafı kullanımı ve faturalama uzlaştırması için yönetim token'larını kullanın |
...
2. Liste yanıtlarında `aisonar.public_contract_summary` ve mevcut olduğunda tam `aisonar.public_contract`'ı okuyun.
...
- Faturalamayı sağlayıcı görev kimlikleri ile karşılaştırmayın. AI Sonar kullanım kayıtlarını kullanın.
