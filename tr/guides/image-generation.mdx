...
Model keşfine başlayın, ardından seçilen modelin AI Sonar sözleşmesini inceleyin:
...
Sohbet dışı modeller için, liste yanıtı `aisonar.public_contract_summary` içerebilir. Model detay sayfaları daha kapsamlı `aisonar.public_contract` sunabilir. Bu alanları doğrulamak için kullanın:
...
- `gpt-image-2` tarzı istekler, OpenAI benzeri `size`, `quality` ve düzenleme alanlarını kullanır. Modelin veya AI Sonar'ın otomatik varsayılanları kullanmasını istiyorsanız isteğe bağlı alanları dışarıda bırakın.
...
- AI Sonar'ı çağırmadan önce kullanıcı girdilerini doğrulayın: istem uzunluğu, görüntü sayısı, URL erişilebilirliği ve dosya türü.
