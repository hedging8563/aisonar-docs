...
AI Sonar'nın Agent-First API'si, hata yanıtlarını AI ajanlarının hemen ayrıştırıp harekete geçebileceği yapılandırılmış ipuçlarıyla zenginleştirir — web aramaları yok, doküman bakışları yok, tahmin yürütme yok.

Her hata yanıtı, standart `error` nesnesi içinde isteğe bağlı alanlar olan `did_you_mean`, `suggestions`, `hint`, `retryable` ve `retry_after` gibi alanları içerir. Bu alanlar geri uyumludur — bunları kullanmayan istemciler hiçbir fark görmez.
...
<Note>
OpenAI-uyumlu uç noktalar, `rate_limit_exceeded`, `upstream_error` ve `all_channels_failed` gibi AI Sonar'nın kararlı genel hata türlerini kullanır. Anthropic-uyumlu ve Gemini-uyumlu uç noktalar kendi yerel yanıt biçimlerini kullanır.
</Note>
...
| Model Sağlayıcı | Önerilen Uç Nokta | Faydası |
|---------------|-------------------|---------|
...
```bash
GET https://api.aisonar.dev/v1/models?category=chat          # Chat modelleri yalnızca
GET https://api.aisonar.dev/v1/models?category=image         # Görüntü üretim modelleri
GET https://api.aisonar.dev/v1/models?tag=coding&category=chat  # Kodlama-optimze chat modelleri
```
